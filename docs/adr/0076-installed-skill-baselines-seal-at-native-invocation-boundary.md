...
现有 Skill 的 invocation-content hash 只能证明模型可见正文相同，不能替代资源树、二进制资源或完整安装包。`dsh-evolve` 因此在原生 Agent `pre-step` 边界观察 durable Skill 调用，不增加 Tool、Prompt、路线菜单或第二套 Session。Host 复用同一 DSH Skill Registry、Agent scope 与 Workspace cwd 重新加载定义，并要求 durable content blocks 与官方 `renderSkillContent()` 输出逐字一致。
