...
入站附件与出站产物仍是两种不同的动作：`FeishuPlatform` 的可选 `sendFile` transport，
只接受名称、字节与 SHA-256，拒绝路径名、空文件、超限和摘要不匹配，并复制校验后的字节以防调用者修改上传内容。
它复用官方 SDK 的单次发送和取消范围，不解析回复、不自动发送，也不自己拥有发送记录；模型 Tool 与原生审批
由 [ADR-0105](0105-feishu-file-delivery-approves-native-snapshots.md) 单独定义，现用 alpha.5 未启用该路径。
不能仅因这个底层接口或原生附件已存在，就声称飞书可以交付文件；更不能扫描模型回复中的本地路径并自动上传。出站必须另有
