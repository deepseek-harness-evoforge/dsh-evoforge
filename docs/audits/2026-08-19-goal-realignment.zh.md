...
| Gateway | `dsh-gateway` 已有 exact route、Workspace/Session/Agent 绑定、入站日志，以及 Telegram/飞书共用的普通文本 outbound intent/journal、按 account 串行、明确限流重试、uncertain 恢复和脱敏健康 | **公共可靠性已收敛；真实平台与 Web 未完成** | 私有 Adapter Delivery Store/worker 已删除；平台 SDK/凭据/发送/卡片留在 Adapter；继续补 transport 聚合、统一 DSH Web、clean-profile 本增量复验与真实 exact 消息 |
...
4. **部分完成：**Gateway 公共普通文本投递/限流响应/健康已收敛；继续补 transport/Web 聚合、飞书真实消息闭环和 DSH Web 权威控制面。
