...
| `dsh-telegram` | 一个静态授权的 Telegram 私聊经 Channel Router 持续使用一个原生 Workspace/Session/Agent，并收到所有完成 turn、Command 结果与一次性 Approval | AS-1 + Router migration implemented；真实 Workspace/Agent Loop、Storage 重启、429、callback、cache parity、Router+Telegram tarball add/boot/remove 已通过；真实 Bot/Hermes paired 待验证 | Channel Router、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation | 0 Tool/Skill/Prompt；平台轮询、approval 和 outbound delivery 在 Adapter，入口 identity/Command 在 Router；普通 Session 0 token | `dsh-evoforge` |
