...
V4.40–V4.45 只深化现有 `dsh-evolve`/`dsh-evolve-web`，不新增插件：existing-Skill 第五 Goal 由第二次 Candidate 不可见治理调用形成独立 Retention Case Pack，并与 Holdout 一起进入 Candidate 已绑定的 v3 Evaluation Envelope；随后只有 authoritative improved Holdout 会经原生 DSH Jobs 执行 exact Retention，Web 只读 Host 权威结果。V4.43 的独立 Host release gate 只在人工批准后发布 inactive Generation，再分步选择未来 Session；V4.44 的 Control/Remote/Web 只投影并调用这个 owner；V4.45 从最终 tarball 验证同一路径，没有第二个 mutation 或审批体系。Canary、晋升、回滚仍属于同一个 `dsh-evolve` 闭环，不拆成新插件、第二调度器、第二 Runtime 或第二审批体系。
