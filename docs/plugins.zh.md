...
| `dsh-gateway` | 为薄渠道 Adapter 提供常驻身份授权、Host 配对、Workspace/Session 绑定、文本/原生图片入站、路由、投递幂等、保守恢复、限流响应和 DSH Web 控制面 | exact route、持久 ingress/outbound、动态 trust grant、hashed pending code、Workspace/live Session/cwd gate、resident零-route transport 健康、同 Host 双 Workspace隔离已回归；最终 tarball 在真实 rc.2 Web 显示 Feishu ready 和批准入口；真实用户消息仍待完成 | WorkspaceRegistry、Agent、Session persistence、Agent presets、Commands、AttachmentStore、Storage Domain、DSH Typert Remote/Client slots/locale | 0 Tool/Skill/Prompt/Command；Web `overview()` 不调用模型，`approvePairing()` 只把外部 code 绑定到已选 native Session；平台 SDK/凭据/发送/重连/卡片留在 Adapter；不拥有 Agent/Session/Goal/Approval 权威，不发明通用 file block | `dsh-evoforge` |
| `dsh-feishu` | 一个飞书 App 通过 resident Gateway 把 exact 私聊持续接入原生 Workspace/Session/Agent，并接收最终回答、Command、一次性 Approval 与主动通知 | implemented；Bundle boot 官方 WebSocket、未知 DM code、Host approval、动态 route、持久 ingress/outbound、429/uncertain、图片、双 Workspace、tarball lifecycle assembled 已测；旧 `/feishu-pair`/两分钟 listener/反向短语/UI 已删除；真实 App 最终包 WebSocket 显示 ready，用户 DM→code→reply 待验证；rc.2 无通用文件/音视频契约，真实内容权限 pending | DSH Gateway、AttachmentStore、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation、DSH Client slots/locale/remotes | 0 Skill/Prompt；首条未知 DM 不进 Agent；普通 Session 只有 scoped `/feishu` 健康 Command；内容 Tool 仅按独立静态权限启用；外部 `fileKey` 只在 Adapter 下载边界存在 | `dsh-evoforge` |
