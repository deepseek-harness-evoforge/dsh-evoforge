...
|---|---|---|---|---|---|
| `dsh-evolve` | 从自然语言 Goal 与原生 Skill catalog 形成可复核 Capability Map/Gap；仅从 DSH 自身跨 Goal 的失败、纠正、返工、结果、成本与复用证据形成 Skill Opportunity、existing-Skill improvement investigation 与隔离 whole-Skill Candidate；再经独立 admission、exact-Candidate Shadow、review、future-Session 晋升和精确回滚闭环 | 重复 Goal-linked Gap → Opportunity → instruction-only whole-Skill v1 quarantine 已实现；Opportunity v3 只从目标回答的唯一 durable Skill invocation/exact Goal 关联纠正，同版本跨 Goal 纠正只形成 `waiting-for-baseline-bundle` 调查；Delivery Outcome 仍按 stable Goal 非因果关联；外部获取、配置式 targets、活动 Git source/ref、Feedback/Evaluator Draft、Shadow 内 proposer、自动 review expiry 与旧 Retention/canary 编排均已删除；Candidate v2/Lineage v3 绑定 evidence seal，Candidate-independent 治理作者分别只见 admission/holdout；Shadow 只消费 exact DSH-assembled Candidate 且零模型调用；新 Skill 可形成内容寻址 inactive Generation，并通过 future Session/root rollback/restart 验证；现有 Skill 完整 Bundle baseline/Candidate、内部 Retention/canary 重建、真实 provider、完整 outcome 归因、Hermes paired 与长期率待验证 | Skill、Session、Goal、Feedback、Storage、Jobs、Commands、Tools result、FS、Shell、Approval、Token Meter、Web | `report_capability_gap(name)` 只报告 Host 可复核缺口；同 Goal retry、一次成功、模型自评和外部资料均不是自我发现证据；禁止占位 baseline；Candidate 无安装/激活/发布权；用户不选路径；author、治理和执行面隔离；当前 Session 固定；付费不确定结果不盲重试 | `dsh-evoforge` |
