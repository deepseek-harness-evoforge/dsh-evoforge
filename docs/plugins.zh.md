...
| `dsh-feishu` | 一个飞书 App 通过 resident Gateway 把 exact 私聊持续接入原生 Workspace/Session/Agent，并接收最终回答、Command、一次性 Approval 与主动通知 | implemented；Bundle boot 官方 WebSocket、未知 DM code、Host approval、动态 route、持久 ingress/outbound、429/uncertain、图片、双 Workspace、tarball lifecycle assembled 已测；旧 `/feishu-pair`/两分钟 listener/反向短语/UI 已删除；真实 App 已完成普通文本、原生 `/new`、普通文本三次入站/回复及 Host 重启恢复；真实 Approval/Schedule/group/failure/long-soak 与内容权限 pending；rc.2 无通用文件/音视频契约 | DSH Gateway、AttachmentStore、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation、DSH Client slots/locale/remotes | 0 Skill/Prompt；首条未知 DM 不进 Agent；普通 Session 只有 scoped `/feishu` 健康 Command；内容 Tool 仅按独立静态权限启用；外部 `fileKey` 只在 Adapter 下载边界存在 | `dsh-evoforge` |
