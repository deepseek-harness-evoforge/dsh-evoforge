...
V4.40–V4.47 只深化现有 `dsh-evolve`/`dsh-evolve-web`，不新增插件：existing-Skill 第五 Goal 形成独立 Retention Case Pack，V4.43–V4.45 完成独立发布门及最终包生命周期；V4.46 的 failed-Outcome Canary 仍在同一插件内由原生 Jobs 重放 exact retained pair，只产出无 mutation 权的证据；V4.47 的 rollback writer 是与 evaluator 分权的 Host gate，并通过同一 Control/Remote/Web 人工确认。Canary、晋升、回滚属于同一个 `dsh-evolve` 闭环，不拆成新插件、第二 Runtime 或第二审批体系。
