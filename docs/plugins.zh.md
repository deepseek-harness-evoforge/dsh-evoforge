...
V4.40–V4.42 只深化现有 `dsh-evolve`/`dsh-evolve-web`，不新增插件：existing-Skill 第五 Goal 由第二次 Candidate 不可见治理调用形成独立 Retention Case Pack，并与 Holdout 一起进入 Candidate 已绑定的 v3 Evaluation Envelope；随后只有 authoritative improved Holdout 会经原生 DSH Jobs 执行 exact Retention，Web 只读 Host 权威结果。Canary、晋升和回滚仍属于同一个 `dsh-evolve` 闭环，不拆成新插件、第二调度器或第二 Runtime。
