...
|---|---|---|---|---|---|
| `dsh-evolve` | 从自然语言 Goal 与原生 Skill catalog 形成可复核 Capability Map/Gap；仅从 DSH 自身跨 Goal 的失败、纠正、返工、结果、成本、复用、Retention 和回滚证据形成 Skill Opportunity 与隔离 whole-Skill Candidate；再经独立 admission、Shadow、Retention、review、future-Session 晋升和精确回滚闭环 | P0A–P1.21 治理底座和重复 Goal-linked Gap → Opportunity → instruction-only whole-Skill v1 quarantine 已实现；Opportunity v2 已保守关联唯一 Session Skill 的纠正引用与 exact Goal revision Skill 的交付结果，固定无因果/无资格影响；外部获取/Agent Skills/runtime research Candidate 活动实现已删除；exact invocation、返工/成本/复用/Retention/回滚、真实 provider 独立整链、Hermes paired 与长期率待验证 | Skill、Session、Goal、Feedback、Storage、Jobs、Commands、Tools result、FS、Shell、Approval、Token Meter、Web | `report_capability_gap(name)` 只报告 Host 可复核缺口；同 Goal retry、一次成功、模型自评和外部资料均不是自我发现证据；Candidate inactive/quarantined/unevaluated/never-executed，不能 install/activate/publish；当前 Session 固定；评测治理面与 proposer 隔离；付费不确定结果不盲重试 | `dsh-evoforge` |
