...
|---|---|---|---|---|---|
| `dsh-evolve` | 从自然语言 Goal 与完整原生 Skill catalog 形成可复核 Capability Map/Gap；从显式授信 local Git 或 Agent Skills v0.2 索引发现并安全隔离单文件/归档 whole-Skill；从 durable Gap 派生跨 Goal 需求聚类，并在部署者 exact target、持久日预算和原生 Jobs 下把无候选同名 cluster 生成成 instruction-only 隔离 Candidate；同时从真实 outcome/反馈形成 Case/Evaluator Draft、Shadow、Retention、canary、review、future-Session 晋升与精确回滚 | P0A + P0B + P0C + P1.1–P1.21 + P2D.1 + V4-1–V4-6 implemented；可信发现、archive 安全隔离、whole-Skill admission/Shadow、跨 Goal 聚类与首个有界生成调度 implemented；官方/论文/开源检索、任意市场、archive/多文件组合、真实 provider/Hermes paired 与长期率待验证 | Skill、Session、Goal、Feedback、Storage、Jobs、Commands、Tools result、FS、Shell、Approval、Token Meter | `report_capability_gap(name)` 只报告 Host 可复核缺口；来源正文与生成正文不进 Web；unknown/跨域/摘要不符/不安全 archive 均 abstain；同 Goal retry 不制造 cluster，冲突 Candidate fail closed；cluster 本身只有 evidence authority，author target 只能写 quarantine，不能 install/activate/publish；当前 Session 固定；评测/holdout/policy 与 proposer 隔离；付费不确定结果不盲重试 | `dsh-evoforge` |
