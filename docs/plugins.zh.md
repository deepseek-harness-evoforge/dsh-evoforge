...
|---|---|---|---|---|---|
| `dsh-evolve` | 从自然语言 Goal 与原生 Skill catalog 形成可复核 Capability Map/Gap；仅从 DSH 自身跨 Goal 的失败、纠正、返工、结果、成本、复用、Retention 和回滚证据形成 Skill Opportunity、existing-Skill improvement investigation 与隔离 whole-Skill Candidate；再经独立 admission、Shadow、Retention、review、future-Session 晋升和精确回滚闭环 | P0A–P1.21 治理底座和重复 Goal-linked Gap → Opportunity → instruction-only whole-Skill v1 quarantine 已实现；Opportunity v3 只从 feedback 目标回答的唯一 durable Skill invocation/exact Goal 关联纠正，V4.21 增加 exact invocation-content hash 并仅对同版本跨 Goal 纠正形成 `waiting-for-baseline-bundle` 调查；Delivery Outcome 仍按 stable Goal 非因果关联；外部获取/runtime research Candidate、配置式 Skill targets、活动 Git source/hidden ref/repository fallback 均已删除；Candidate v2/Lineage v3 显式绑定生成前 evidence seal，Candidate-independent 治理作者分别只见 admission/holdout 子集，零 proposer 校准并原子安装 Envelope v4；真实 capability-absent baseline 已贯穿 admission/assembled Shadow/Review/crash resume；新 Skill 只通过内部 exact Bundle 形成内容寻址 inactive Generation，并通过真实 DSH future Session/root rollback/restart验证；独立 Retention 与 sealed canary 已对 exact absent parent/whole-Skill/lineage 做无 Git 真实 DSH replay；未装配历史模块、现有 Skill 完整 Bundle baseline/Candidate、治理包真实 provider assembled 评估、返工/成本/复用/回滚归因、Hermes paired 与长期率待验证 | Skill、Session、Goal、Feedback、Storage、Jobs、Commands、Tools result、FS、Shell、Approval、Token Meter、Web | `report_capability_gap(name)` 只报告 Host 可复核缺口；同 Goal retry、一次成功、模型自评和外部资料均不是自我发现证据；invocation hash 不是完整 Bundle；禁止占位 Skill baseline；Candidate inactive/quarantined/unevaluated/never-executed，不能直接 install/activate；只有独立评测后的明确 review 可生成 inactive Generation；author/evaluation policy 都不能预选 Skill；当前 Session 固定；评测治理面与 proposer 隔离；付费不确定结果不盲重试 | `dsh-evoforge` |
...
| `dsh-gateway` | 为多个薄渠道 Adapter 提供统一的身份标准化、Workspace/Session 绑定、路由、投递意图、幂等、保守恢复、限流响应和 DSH Web 健康视图 | 已直接替换旧 `dsh-channel-router` 且无转发包；exact route、持久 ingress、Command 一次执行、Telegram/飞书公共 outbound intent/journal、按 account 串行、明确限流重试、uncertain 恢复、脱敏 transport observation、同 Host 双 Workspace 隔离已回归；同包 Client Module 的真实 DSH 浏览器读取/刷新/Host 停机清空旧快照/同端口恢复已通过；真实 exact 消息仍待完成 | WorkspaceRegistry、Agent、Session persistence、Agent presets、Commands、Storage Domain、DSH Typert Remote/Client slots/locale | 0 Tool/Skill/Prompt/Command；Web 只有无参数只读 `overview()`，不调用模型；平台 SDK/凭据/实际发送/卡片 UI/重连策略留在 Adapter；Gateway 只校验和聚合 exact-route transport facts，不拥有 Agent/Session/Goal/Approval 权威，也不声称全局配额或 exactly-once；普通 Session 0 token | `dsh-evoforge` |
