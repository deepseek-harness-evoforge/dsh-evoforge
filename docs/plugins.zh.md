...
| `dsh-telegram` | Telegram 私聊经 DSH Gateway 持续使用一个原生 Workspace/Session/Agent；支持精确静态路由和 Hermes 式 Host 配对，陌生私聊首条消息只返回一次性配对码，管理员批准后下一条消息才进入 Session；Web 控制台显示配对/路由和持久投递 | AS-1 + Gateway migration implemented；真实 Workspace/Agent Loop、Storage 重启、429、callback、cache parity、Gateway+Telegram tarball lifecycle、与飞书同 Host 双 Workspace 重启隔离、实际 tarball 的 loopback 静态 Surface 浏览器刷新/reload 已通过；新增 pairing assembled 纵切验证 unknown DM→code→Host approve→next-message dispatch；真实 Bot/Hermes paired 与外部 route 浏览器门待验证 | DSH Gateway、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation、DSH API Remotes、Control Center Surface | 0 Tool/Skill/Prompt；一个只读 Web Surface；平台轮询、发送映射和 Approval 在 Adapter，入口 identity/Command 与普通文本 outbound 在 Gateway；pairing 模式不向 attention 暴露未选 Workspace 的通知 route；普通 Session 0 token | `dsh-evoforge` |
