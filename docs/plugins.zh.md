...
|---|---|---|---|---|---|
| `dsh-evolve` | 从自然语言 Goal 与原生 Skill catalog 形成可复核 Capability Map/Gap；仅从 DSH 自身跨 Goal 的失败、纠正、返工、结果、成本与复用证据形成 Skill Opportunity、existing-Skill improvement investigation 与隔离 whole-Skill Candidate；再经独立 admission、exact-Candidate Shadow、Retention、review、future-Session 晋升和精确回滚闭环 | 重复 Goal-linked Gap → Opportunity → instruction-only whole-Skill v1 quarantine 已实现；Opportunity v3 只从目标回答的唯一 durable Skill invocation/exact Goal 关联纠正，同版本跨 Goal 纠正现逐一重验 immutable Session/invocation reference，只有全部指向同一调用时完整 baseline Bundle 才获得无发布权的 protected authoring 资格；Delivery Outcome 仍按 stable Goal 非因果关联；外部获取、配置式 targets、活动 Git source/ref、Feedback/Evaluator Draft、Shadow 内 proposer、自动 review expiry 与旧 Retention/canary 编排均已删除；Candidate v2/Lineage v3 绑定 evidence seal，Candidate-independent 治理作者分别只见 admission/holdout/可选第五 Goal Retention；Shadow 只消费 exact DSH-assembled Candidate，promotable 结果在同一 Jobs 任务进入内容寻址 Retention paired Trial，均为零 proposer；新 Skill 可形成内容寻址 inactive Generation，并通过 future Session/root rollback/restart 验证；独立 Host gate 只允许 exact retained Candidate 晋升未来 Session，Web 已验证 eligible/blocked/失败恢复；失败 Outcome 另经原生 Jobs 触发 exact sealed Canary，只产出 keep/review/rollback-eligible 且自身无 pointer 权限；独立 rollback gate 现重验 exact Canary 并以 expected-active compare 执行人工确认的 future-Session 回滚，Command/Remote/Web 无直接 Store 旁路；现有 Skill protected author/Candidate/paired evaluation、真实 provider/浏览器 rollback、完整 outcome 归因、Hermes paired 与长期率待验证 | Skill、Session、Goal、Feedback、Storage、Jobs、Commands、Tools result、FS、Shell、Approval、Token Meter、Web | `report_capability_gap(name)` 只报告 Host 可复核缺口；同 Goal retry、一次成功、模型自评和外部资料均不是自我发现证据；禁止占位 baseline；Candidate/Retention/Canary verdict 无安装/激活/发布权；用户不选路径；author、治理、执行和 mutation 面隔离；当前 Session 固定；付费不确定结果不盲重试 | `dsh-evoforge` |
