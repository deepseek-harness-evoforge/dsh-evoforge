...
| `dsh-feishu` | 一个飞书 App 的 exact 私聊/群聊通过 Router 持续使用原生 Workspace/Session/Agent，并接收最终回答、Command、一次性 Approval 与主动通知 | implemented；官方 SDK WebSocket、真实 DSH Agent Loop、持久 ingress/outbound、429/uncertain、同 Host 双 Workspace 重启隔离、tarball lifecycle 已测；真实 App 身份请求及经标准 HTTPS proxy 的 WebSocket 握手通过，exact chat/user 消息闭环待验证 | Channel Router、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation | 0 Tool/Skill/Prompt；平台协议、卡片与投递 worker 在 Adapter；普通 Session 0 token | `dsh-evoforge` |
