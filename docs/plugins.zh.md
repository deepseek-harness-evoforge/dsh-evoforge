...
|---|---|---|---|---|---|
| `dsh-evolve` | 从真实任务结果产生 inactive Skill Candidate，独立评测，随后为未来 Session 安全晋升或回滚 | P0A Shadow + P0B.1 release kernel + P0B.2a explicit resume；P0C 才提供用户控制面 | Skill、Session、Goal、Feedback、Storage、Jobs、FS、Shell、Approval、Token Meter | 无新增 Evolve Tool/Prompt；active Generation 复用原生 Skill catalog/body，并按 Session 固定 | `dsh-evoforge` |
