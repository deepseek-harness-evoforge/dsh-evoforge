...
| `dsh-doctor` | 一次命令读取当前原生 Loader 与现有 Gateway 脱敏 transport health，把插件及必需飞书/Telegram 连接就绪归约成三态、阻塞项与下一步；只诊断不修复 | implemented；最终 tarball add/dump/boot 中准确观察 degraded，经 Cordis reload 观察 ready，remove 清理已通过；损坏快照 fail closed | Commands、Loader、可选 DSH Gateway 只读服务 | 一个 human Command；0 Tool/Skill/Prompt；无平台调用、凭据读取、轮询、健康副本或自动修复 | `dsh-evoforge` |
...
| `dsh-gateway` | 为薄渠道 Adapter 提供常驻身份授权、Host 配对、Workspace/Session 绑定、文本/原生图片入站、路由、投递幂等、保守恢复、限流响应和 DSH Web 控制面 | exact route、持久 ingress/outbound、动态 trust grant、hashed pending code、Workspace/live Session/cwd gate、resident零-route transport 健康、同 Host 双 Workspace隔离已回归；最终 tarball 在真实 rc.2 Web 完成真实飞书 DM→code→Host approve→三次 native Session/回复，journal 3/3 且零异常；Host 冷启动恢复 exact grant/Session/journal/transport，无重复投递 | WorkspaceRegistry、Agent、Session persistence、Agent presets、Commands、AttachmentStore、Storage Domain、DSH Typert Remote/Client slots/locale | 0 Tool/Skill/Prompt/Command；Web `overview()` 不调用模型，`approvePairing()` 只把外部 code 绑定到已选 native Session；平台 SDK/凭据/发送/重连/卡片留在 Adapter；不拥有 Agent/Session/Goal/Approval 权威，不发明通用 file block | `dsh-evoforge` |
| `dsh-feishu` | 一个飞书 App 通过 resident Gateway 把 exact 私聊持续接入原生 Workspace/Session/Agent，并接收最终回答、Command、一次性 Approval 与主动通知 | implemented；Bundle boot 官方 WebSocket、未知 DM code、Host approval、动态 route、持久 ingress/outbound、429/uncertain、图片、双 Workspace、tarball lifecycle assembled 已测；旧 `/feishu-pair`/两分钟 listener/反向短语/UI 已删除；真实 App 已完成普通文本、原生 `/new`、普通文本三次入站/回复及 Host 重启恢复；动态 paired route 已进入 Host notice seam，assembled Approval `allowed-once`/notice 通过；真实 Approval/Schedule/group/failure/long-soak 与内容权限 pending；rc.2 无通用文件/音视频契约 | DSH Gateway、AttachmentStore、Agent、Session、Commands、Approval、Storage Domain、Goal/Schedule continuation、DSH Client slots/locale/remotes | 0 Skill/Prompt；首条未知 DM 不进 Agent；普通 Session 只有 scoped `/feishu` 健康 Command；内容 Tool 仅按独立静态权限启用；外部 `fileKey` 只在 Adapter 下载边界存在 | `dsh-evoforge` |
