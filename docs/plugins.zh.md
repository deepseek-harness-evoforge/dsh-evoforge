...
| `dsh-software-delivery` | 把原生 Goal 交付为隔离、遵循仓库规范、验证过的 commit 和可选 Draft PR | P2A.1 verifier + P2B.1 Goal completion + P2C.1 idempotent Draft PR implemented；Evolve 集成待完成 | Goal、ToolGoal、Skill、Shell、Sandbox、Approval | 一个稳定 Skill catalog entry；正文按需；同一个 ≤2 KiB 稳定 Tool Schema 覆盖可选 PR；无 Prompt | `dsh-evoforge`，成熟后可独立采用 |
