...
本身就是 opt-in；chat/user/token/Agent 继续只在 `dsh-telegram` 的现有配置中出现。
...
首版不加按钮、催办、优先级、模板系统、订阅、第二渠道或日程。先测 discovered rate、
time-to-first-review、stale review count 与真实 Bot 多日投递；只有第二个真实渠道有独立用户结果和权限
边界时，才重新评估公共 notification seam。
