...
| 自主 Skill 发现 | 自然语言 Goal 自动命中已验证能力；无能力时形成缺口并从可信来源产生完整候选 | 原生目录/路由证据、可证伪 Gap、部署者授信本地 Git whole-Skill 的 exact-first 确定性语义发现、Agent Skills v0.2 `skill-md` 同源/摘要/identity 校验与 durable quarantine、同 Workspace 跨 Goal 需求聚类、固定身份隔离重放、零模型确定性准入、独立 exact-Candidate assembled holdout 自动 handoff 与 Web/Review 解释 implemented；archive/任意市场与官方资料搜索、生成、cluster-driven 调度、迁移/成本门禁与 paired benchmark pending |
