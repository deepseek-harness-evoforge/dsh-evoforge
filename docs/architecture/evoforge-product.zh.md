...
| 自进化 | 内部经验发现、独立治理、inactive Candidate、可证明晋升 | Goal-linked Gap → cross-Goal Opportunity → evidence seal → Candidate-independent admission/holdout/Retention Case Pack → exact-Candidate assembled Shadow → durable Retention verdict → review → content-addressed inactive Generation → exact retained Promotion Eligibility → future-Session selection → failed-Outcome sealed canary evidence 已形成活动纵切；历史 Feedback/Evaluator Draft、static target、Shadow proposer、自动过期和旧 Retention/canary 已撤销；existing-Skill 完整 Bundle、独立 rollback gate、真实 provider、陌生用户与长期效果 pending |
