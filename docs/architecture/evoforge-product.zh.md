...
existing-Skill 路径不复用 capability-absent subject。Host 在调用时封存 exact installed Skill、从当前纠正预隔离 authoring/admission/holdout/Retention；Candidate-blind 治理在 proposer 前以两个独立单样本调用形成 calibrated `skill-tree` Holdout 与可选 Retention Case Pack，并把两者共同封入 Candidate 已绑定的 v3 Evaluation Envelope。受保护作者只产生有界指令 diff，Host 组装完整继承 baseline 的 Candidate。结构准入只证明 exact 双树和 declared diff；独立原生 DSH Jobs 依次执行 exact Holdout，并只从权威 improved Holdout 执行 exact Retention。两者都要求 calibration、assembled、composition 与输入完整性全部成立才产生四象限效果判决；未知付费结果不盲重试，结果无发布权。V4.39 已完成 Holdout 最终 tarball 的真实 DSH Web 生命周期；V4.42 已完成 Retention 的权威投影与浏览器生命周期；V4.43 由独立 Host gate 重验 exact 证据和完整 sealed Bundle，人工 approve 只发布 inactive Generation，另一动作才选择未来 Session；V4.44 的 Control/Remote/Web 只投影并调用同一 owner，不增加 Store/pointer writer；V4.45 已完成 release 最终 tarball 浏览器。真实 provider Trial 与 failed-Outcome Canary 仍 pending。
