...
来源搜索/获取以及 cluster-driven 的 instruction-only Candidate 生成已有首个纵切；官方资料检索、任意市场和 archive/多文件组合仍待实现。在线快环只收集可归因信号和小步候选，
...
| 自主 Skill 发现 | 自然语言 Goal 自动命中已验证能力；无能力时形成缺口并从可信来源产生完整候选 | 原生目录/路由证据、可证伪 Gap、local Git/Agent Skills v0.2 可信发现与安全 quarantine、跨 Goal 聚类、exact target + 日预算 + 原生 Jobs 的 instruction-only Candidate 生成、固定身份隔离重放、确定性准入、独立 assembled holdout 与 Web 解释 implemented；官方/论文/开源检索、任意市场、archive/多文件组合、真实 provider、迁移/成本门禁与 paired benchmark pending |
