...
Capability Map、两种可区分的 Capability Gap、可信本地 Git whole-Skill 的 exact-first 确定性语义发现、隔离准入
...
并先落盘，后台再形成 inactive 候选。外部来源搜索/获取、无候选时生成和跨 Goal 慢环仍待实现。在线快环
...
| 自主 Skill 发现 | 自然语言 Goal 自动命中已验证能力；无能力时形成缺口并从可信来源产生完整候选 | 原生目录/路由证据、可证伪 Gap、部署者授信本地 Git whole-Skill 的 exact-first 确定性语义发现、固定身份隔离重放、零模型确定性准入、独立 exact-Candidate assembled holdout 自动 handoff 与 Web/Review 解释 implemented；外部来源搜索/获取、生成、迁移/成本门禁与 paired benchmark pending |
