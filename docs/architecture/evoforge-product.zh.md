...
| 自主 Skill 发现 | 自然语言 Goal 自动命中已验证能力；无能力时形成缺口并从可信来源产生完整候选 | 原生目录/路由证据、可证伪 Gap、部署者授信本地 Git whole-Skill、固定身份隔离重放、零模型确定性准入与 Web 解释 implemented；外部来源、生成、assembled/holdout Trial 与 paired benchmark pending |
