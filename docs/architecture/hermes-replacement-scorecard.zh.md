...
> 历史生态审计基线保留用于既有证据；2026-09-04 Hermes 当前远端 revision 已增量复核为
> `29d0cc2602e01943ab300c0382fc9d97efb376da`，旧 `63279301…` 仅作为 epoch-3 历史锁定，详见[当前 revision 增量复核](../research/hermes-current-revision-2026-09-04.zh.md)。
...
| `EV-1` 可证明进化 | 重复错误被 Skill Candidate 修正并通过未见 case | known-bad 被拒；真实修正通过 final-test；原 Session 与 active Skill 不变；Decision 可重放 | final-test 改善、false promotion、每次减少返工的成本 | [一个确定性 Hermes paired release-control slice](../evidence/ev-1-hermes-paired-benchmark.zh.md) 已达到 `better for deterministic Skill-correction release control`；[V5.123 当前 alpha.5 epoch-2](../evidence/v5-123-hermes-ev1-alpha5-epoch-2-2026-09-04.zh.md) 与 [V5.135 当前 Hermes epoch-3](../evidence/v5-135-hermes-current-ev1-epoch-3-2026-09-04.zh.md) 在同一窄场景重新冻结并通过；P0A–P1.21 其余实现不变；真实 provider、同任务分布的长期改善/误晋升/单位成本数据 pending |
...
V5.224 起，SD-1、LC-1、AS-1 的 current epoch 与 EV-1 current epoch-4 都可通过
`pnpm benchmark:hermes:current` 在显式审计支持组合上严格复跑；这只更新 benchmark 的 revision 复现入口，不
改变上表对真实 Provider、真实渠道、长期效果或整体 `Hermes upper alternative` 的未完成判断。详见
[V5.224](../evidence/v5-224-current-hermes-benchmark-suite-2026-09-04.zh.md)。
