...
|---|---|---|---|---|
| `DS-1` 自主能力发现 | 用户只给自然语言 Goal，系统无需开场菜单即可调用正确的现有 Skill，或形成真实缺口并发现/生成可评测候选 | Capability Map 可解释；现有能力优先；来源/scope/version/hash/权限可追踪；whole-Skill 候选 inactive；错误路由和未知代码 fail closed；候选无法读取治理面 | 首次成功率、Skill discovery recall/precision、错误调用、人工选路/纠正、transfer、negative transfer、token/时延 | [原生目录、真实 Gap、可信 whole-Skill 隔离候选、确定性准入与独立 exact assembled holdout handoff](../evidence/ds-1-trusted-whole-skill-discovery.zh.md)、[本地可信语义回退](../evidence/v4-2-trusted-semantic-skill-search.zh.md)、[跨 Goal 需求聚类](../evidence/v4-3-cross-goal-gap-demand.zh.md) 与 [Agent Skills v0.2 外部制品获取](../evidence/v4-4-agent-skills-index-discovery.zh.md) implemented；archive/任意市场、官方/论文搜索和生成、cluster-driven 调度、迁移/成本门禁及 paired benchmark pending |
