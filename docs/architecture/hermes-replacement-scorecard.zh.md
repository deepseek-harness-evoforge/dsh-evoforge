...
| `AS-1` 通用助理 | 一个真实消息或日程工作流从触发到交付完整闭环 | 渠道路由、会话连续性、审批、幂等投递、失败重试和结果可查 | 成功投递、重复投递、人工步骤、时延 | [确定性 Telegram approval paired slice](../evidence/as-1-hermes-paired-benchmark.zh.md) 在错误身份/重放解析动作上 `0:0` 打平；Telegram/飞书真实 DSH routes、durable journals、429、[有界 send/dispose](../evidence/v5-7-bounded-channel-delivery.zh.md)与 cache parity implemented；[V5.8](../evidence/v5-8-real-feishu-acceptance-gate.zh.md) 已把 exact 飞书真实入站/回复/Command/Approval/卸载/readback 编成 hard gate，但 direct/group 都是 `NOT_RUN`；真实 Bot/App delivery、陌生安装和同模型消息 paired pending |
