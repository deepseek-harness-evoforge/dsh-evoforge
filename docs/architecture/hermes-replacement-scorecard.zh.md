...
| `AS-1` 通用助理 | 一个真实消息或日程工作流从触发到交付完整闭环 | 渠道路由、会话连续性、审批、幂等投递、失败重试和结果可查 | 成功投递、重复投递、人工步骤、时延 | Telegram/飞书 + `dsh-evolve-attention` 非阻塞决策提醒 implemented：真实 DSH routes、durable journals、429、cache parity 与 package 边界已测；真实 Bot/App soak、陌生安装和 Hermes paired pending |
