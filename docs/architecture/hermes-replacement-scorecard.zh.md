...
| `UI-1` 非阻塞控制 | 用户能查看状态、证据、成本、权限并 pause/review/rollback | 不调用模型即可读取权威状态；不回复 review 不影响原会话；动作结果明确 | 找到并完成控制动作的时间、误操作、前台阻塞 | P0C.1 release + P0C.2 review + P0C.3 pause/resume + P0C.4 exact bounded diff implemented；权限效果投影与真实可用性数据 pending |
