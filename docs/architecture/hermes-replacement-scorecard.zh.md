...
| `RM-1` 可删除性 | 用户禁用或删除 EvoForge 后仍可使用原生 DSH | 无私有事件阻断 Session；Goal 可读；无 watcher/process/config 残留 | 卸载成功率、残留数、数据可导出性 | Evolve/Delivery/Doctor/Goal Continuity packed artifact lifecycle + `dsh-resident` stop/unit removal/no third restart + native Session/Goal resume implemented；保留日志明确披露，第三方复跑 pending |
