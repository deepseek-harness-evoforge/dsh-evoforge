...
> 状态：implemented；默认关闭随 P1.14，启用自动 Target 后默认每个 Target 每 UTC 日最多预留 1 次自动尝试

## 用户结果

> 常驻 Agent 即使连续收到大量明确纠正或在付费边界崩溃重启，也不能无限自动消耗 proposer token；操作者可以在 Commands/Web 解释今天用了多少、还剩多少，原会话始终继续。

P1.15 只收紧 P1.14 的自动入口。每次显式 `/evolve feedback … shadow …` 或 Web 确认仍是一项新的人工授权，不受自动 cap 限制；Promotion、Retention、Canary 和 rollback 规则不变。
...
`maxAttemptsPerUtcDay` 默认 `1`，范围 `1..20`。它限制“可能进入一次 proposer 的自动尝试预留数”，不是实际账单。单次输入/输出 token 仍由 exact Case Pack manifest 限制，因此一个 Target 的日 proposer 上界可解释为：
...
one exact explicit signal
  ...
  └─ durable 0600 reservation
       └─ private Draft → native Job → at most one proposer request
```

- 预留位于 Target 已有 owned run root 的 `.automatic-evolution-budget-v1/current.json`；文件只保留当日最多 20 个条目，UTC 换日时原子替换，长期磁盘状态恒定有界；浏览器和命令不接收或返回路径；
...
- UTC 换日开启新的有界窗口；旧 Signal 即使再次出现，既有 Shadow launch identity 仍阻止重复 proposer；`proposal-pending` 的不确定请求绝不自动重试。
...
全局/跨插件配额、真实货币价格、provider 账单对账、分钟级滚动窗口、团队共享额度、自动增额、Web 配置编辑、显式人工动作限流、自动 evaluator author/qualification 和跨主机 lease 均不进入 P1.15。
