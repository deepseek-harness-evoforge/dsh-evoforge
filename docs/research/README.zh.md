...
本目录记录 EvoForge 设计所依据的源码审计与用户需求证据。项目结论以固定 revision 为准，避免把持续变化的上游仓库误写成永久事实。报告正文中的 `oh-my-dsh` 是本项目采用 EvoForge 组织名之前的工作名称，不代表第二套产品。
...
8. [可证明自进化设计](../architecture/evolution-design.zh.md)：查看 Generation 固定、Learning Signal、Candidate、Trial、晋升、回滚、缓存与 P0 测试规格。
...
- 当前只完成研究与设计文档，不代表用户已经确认进入 P0 实现。
