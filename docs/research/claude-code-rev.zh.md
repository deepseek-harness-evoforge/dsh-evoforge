...
> 审计对象：`/Users/my/harness/claude-code-rev`
...
- README 明确写明代码树主要由 source map 恢复，并且“不是原始 upstream repository state”：[README.md](/Users/my/harness/claude-code-rev/README.md:8)。
- 包版本是 `999.0.0-restored`，描述也是 “reconstructed from source maps”：[package.json](/Users/my/harness/claude-code-rev/package.json:3)。
- README 承认 type-only 文件、构建期生成文件、私有包包装、native binding、动态资源等无法完整恢复：[README.md](/Users/my/harness/claude-code-rev/README.md:35)。
- 实际 Git remote 是 `zhaoquan219/claude-code-rev`；`package.json` 虽填写 Anthropic 仓库 URL，但这不是当前仓库的来源证明。
- SDK 的 `tool()`、`createSdkMcpServer()`、`query()` 和持久会话 API 仍直接抛出 `not implemented`：[agentSdkTypes.ts](/Users/my/harness/claude-code-rev/src/entrypoints/agentSdkTypes.ts:73)。
- MCP Skill 获取函数固定返回空数组：[mcpSkills.ts](/Users/my/harness/claude-code-rev/src/skills/mcpSkills.ts:1)。
- Chrome MCP、Computer Use 和若干 native 依赖使用本地 shim；README 也将其称为 compatibility/degraded implementation：[package.json](/Users/my/harness/claude-code-rev/package.json:24)。
- 内建插件注册表目前没有注册任何插件，只留下迁移脚手架：[bundled/index.ts](/Users/my/harness/claude-code-rev/src/plugins/bundled/index.ts:20)。
...
入口很薄：`bootstrap-entry.ts` 初始化运行时宏后动态导入 CLI：[bootstrap-entry.ts](/Users/my/harness/claude-code-rev/src/bootstrap-entry.ts:1)。CLI 又用动态 import 分流 `--version`、Chrome MCP、Computer Use、daemon worker、remote control 等快速路径，以降低普通启动的模块求值成本：[cli.tsx](/Users/my/harness/claude-code-rev/src/entrypoints/cli.tsx:28)。

主入口的关键职责是：

- 确定 interactive/headless 模式和 permission mode；
- 读取设置、环境、workspace trust 和企业策略；
- 提前并行读取 MCP 配置，但在交互模式中把真正连接延后到 trust 建立之后：[main.tsx](/Users/my/harness/claude-code-rev/src/main.tsx:1805)；
- 在命令和 Agent 扫描前注册 bundled skills 和 builtin plugin，避免并行初始化时缓存空集合：[main.tsx](/Users/my/harness/claude-code-rev/src/main.tsx:1925)；
- 构造内建工具池、插件内容、Agent 定义和 MCP 工具；
- 进入 Ink REPL，或把全部运行参数传给 `runHeadless`：[main.tsx](/Users/my/harness/claude-code-rev/src/main.tsx:2832)。

这是一套成熟的“交互式 Agent Loop + 丰富扩展面”，但不是 durable workflow engine。长期状态的主要抽象仍是 conversation/session，而不是可单独调度、租约化和重放的工作步骤。

## 3. 核心控制流：一次请求如何运行

`query()` 是异步生成器，真正的持续循环位于 `queryLoop`：[query.ts](/Users/my/harness/claude-code-rev/src/query.ts:219)。单轮可以概括为：
...
6. 创建 `StreamingToolExecutor`，开始模型流式请求：[query.ts](/Users/my/harness/claude-code-rev/src/query.ts:545)。
...
Tool 执行的关键顺序是“Hook 可以提出修改或阻断，但最终仍进入中央权限决策，再调用工具”。实际执行在：[toolExecution.ts](/Users/my/harness/claude-code-rev/src/services/tools/toolExecution.ts:916)。
...
完整接口见：[Tool.ts](/Users/my/harness/claude-code-rev/src/Tool.ts:362)。`ToolResult` 还可以追加消息、修改后续 context，并透传 MCP `_meta/structuredContent`：[Tool.ts](/Users/my/harness/claude-code-rev/src/Tool.ts:321)。

工具池组装有三个值得学习的动作：

1. 先执行 blanket deny，把被禁止的工具从模型可见集合中删除，而不是等调用后拒绝：[tools.ts](/Users/my/harness/claude-code-rev/src/tools.ts:262)。
2. 再按模式、环境、平台和 feature gate 过滤。
3. 合并内建与 MCP 工具，按稳定分区排序并去重，名称冲突时内建工具优先：[tools.ts](/Users/my/harness/claude-code-rev/src/tools.ts:329)。
...
- `backfillObservableInput` 明确保护原始 API-bound input，避免修改 prompt-cache 对应内容：[Tool.ts](/Users/my/harness/claude-code-rev/src/Tool.ts:474)。
...
Manifest 总 schema 见：[schemas.ts](/Users/my/harness/claude-code-rev/src/utils/plugins/schemas.ts:884)。加载器既支持 `.claude-plugin/plugin.json`，也能按约定目录自动发现 commands、agents、skills、output styles 和 hooks。
...
企业 managed 设置能阻止本地 `--plugin-dir` 覆盖受管插件：[pluginLoader.ts](/Users/my/harness/claude-code-rev/src/utils/plugins/pluginLoader.ts:2995)。启动消费者默认使用 cache-only loader，避免交互启动被 marketplace 网络和 git clone 阻塞；显式刷新才运行 full loader：[pluginLoader.ts](/Users/my/harness/claude-code-rev/src/utils/plugins/pluginLoader.ts:3111)。加载完成后会检查依赖，把缺依赖插件降级为 disabled，但依赖目前是 presence check，不是拓扑启动图：[pluginLoader.ts](/Users/my/harness/claude-code-rev/src/utils/plugins/pluginLoader.ts:3189)。

它仍不是严格意义的“一切皆插件”：

- 外部插件没有 code-native `tools` 组件；新增工具主要经 MCP 接入。
- 核心工具仍在 `getTools()` 中硬编码装配。
- builtin plugin 类型只接受 skills、hooks、MCP servers 等少数组件：[plugin.ts](/Users/my/harness/claude-code-rev/src/types/plugin.ts:18)。
- `PluginComponent` 联合没有 MCP、LSP 和 settings，与 `LoadedPlugin` 能承载的字段不一致：[plugin.ts](/Users/my/harness/claude-code-rev/src/types/plugin.ts:48)。
- 插件 settings 当前只允许 `agent` 一个键：[schemas.ts](/Users/my/harness/claude-code-rev/src/utils/plugins/schemas.ts:852)。
- 内建插件注册函数当前没有注册任何实际插件。
- 错误类型注释承认生产中实际只用了两个粗粒度错误，其他类型仍是计划项：[plugin.ts](/Users/my/harness/claude-code-rev/src/types/plugin.ts:86)。

对 DSH/oh-my-dsh 最有价值的是来源优先级、受管策略、缓存优先启动、依赖降级和可失效缓存；不应复制其“核心仍硬编码、插件只包外围内容”的边界。
...
按 feature gate 或环境可能额外注册：dream、hunter、loop、schedule-remote-agents、claude-api、claude-in-chrome、run-skill-generator。注册事实见：[skills/bundled/index.ts](/Users/my/harness/claude-code-rev/src/skills/bundled/index.ts:24)。

Skill 元数据支持 `allowedTools`、固定 model/effort、禁止模型主动调用、`context: fork`、指定 Agent、Hooks、附带文件和条件路径激活。文件加载覆盖 managed、user、project、`--add-dir` 和 legacy commands；还会沿 cwd 向上发现，距离当前目录更近的 Skill 可覆盖远处定义：[loadSkillsDir.ts](/Users/my/harness/claude-code-rev/src/skills/loadSkillsDir.ts:923)。

安全细节值得注意：Skill 引用文件会创建 0700 临时目录，使用 `O_NOFOLLOW | O_EXCL`，并拒绝路径穿越：[bundledSkills.ts](/Users/my/harness/claude-code-rev/src/skills/bundledSkills.ts:118)。本地 Skill 可以做 inline shell expansion，但 MCP Skill body 明确不执行 inline shell。`context: fork` 会启动隔离 Agent，而不是简单把 Skill 文本拼到主上下文：[SkillTool.ts](/Users/my/harness/claude-code-rev/src/tools/SkillTool/SkillTool.ts:118)。
...
定义 schema 见：[loadAgentsDir.ts](/Users/my/harness/claude-code-rev/src/tools/AgentTool/loadAgentsDir.ts:56)。Agent 覆盖顺序是 builtin → plugin → user → project → CLI flag → managed，后来源覆盖前来源：[loadAgentsDir.ts](/Users/my/harness/claude-code-rev/src/tools/AgentTool/loadAgentsDir.ts:193)。

`runAgent` 的生命周期比较完整：[runAgent.ts](/Users/my/harness/claude-code-rev/src/tools/AgentTool/runAgent.ts:248)
...
对 oh-my-dsh 的启示不是再造一个 Agent Runtime，而是让 DSH 原生 Subagent、Goal、Workflow、Session、Jobs 和 Schedule 承担运行；插件只增加可测量的新工作流或 Completion Check。
...
事件枚举见：[coreTypes.ts](/Users/my/harness/claude-code-rev/src/entrypoints/sdk/coreTypes.ts:25)。Hook 实现包括 shell command、prompt、HTTP、Agent verifier、SDK/internal function callback：[hooks.ts](/Users/my/harness/claude-code-rev/src/schemas/hooks.ts:31)。
...
配置来源会合并 snapshot、registered hooks 和当前 session hooks；managed-only 设置能禁止插件及 Skill/Agent frontmatter 绕过策略：[hooks.ts](/Users/my/harness/claude-code-rev/src/utils/hooks.ts:1492)。

安全边界很明确：交互模式下所有 Hook 都要求 workspace trust，因为它们能够执行任意本地命令：[hooks.ts](/Users/my/harness/claude-code-rev/src/utils/hooks.ts:267)。然而非交互模式被视为隐式可信：[hooks.ts](/Users/my/harness/claude-code-rev/src/utils/hooks.ts:286)，这对无人值守、处理不可信仓库的长期 Agent 是明显风险。

对 DSH 最值得吸收的是“版本化事件契约 + typed result”，而不是 Hook 直接执行宿主 shell 的权限模型。oh-my-dsh 应优先复用 DSH Hooks；无变化的 Hook 必须真正 no-op，不能为观察状态持续改写模型可见前缀。

## 9. MCP：把外部能力适配进统一 Tool 控制面

MCP 类型支持 local、user、project、dynamic、enterprise、claudeai、managed 等 scope，以及 stdio、SSE、HTTP、WebSocket、SDK、claude.ai proxy 等 transport：[mcp/types.ts](/Users/my/harness/claude-code-rev/src/services/mcp/types.ts:10)。
...
消息定义和 sidechain 约定见：[sessionStorage.ts](/Users/my/harness/claude-code-rev/src/utils/sessionStorage.ts:128)。
...
恢复入口见：[conversationRecovery.ts](/Users/my/harness/claude-code-rev/src/utils/conversationRecovery.ts:416)，运行态恢复见：[sessionRestore.ts](/Users/my/harness/claude-code-rev/src/utils/sessionRestore.ts:95)。
...
oh-my-dsh 不应因此建立第二套事件溯源或 Workflow 数据库。项目基线要求复用 DSH Session Persistence、Goal、Storage、Jobs、Schedule 和 Workflow。可借鉴的是 append-only evidence、明确 sidechain 和可恢复 checkpoint；实现必须落在 DSH 原生接缝内，并保持状态少量、幂等、可删除。
...
主循环入口见：[query.ts](/Users/my/harness/claude-code-rev/src/query.ts:365)。`compactConversation()` 会触发 PreCompact，生成 summary 和 compact boundary，再恢复必要运行状态：[compact.ts](/Users/my/harness/claude-code-rev/src/services/compact/compact.ts:387)。恢复内容包括近期文件、异步 Agent、plan、Skills、deferred tools、Agent 列表和 MCP instructions；随后重新触发 SessionStart compact 与 PostCompact。媒体内容会先转换为文本 marker，附件和 Skill 恢复都有单项及总量预算。

Autocompact 还有连续失败计数和三次失败 circuit breaker：[autoCompact.ts](/Users/my/harness/claude-code-rev/src/services/compact/autoCompact.ts:51)。
...
对 oh-my-dsh，应把 Compaction 保持为 DSH Core 的上下文职责；Evolution Loop 只消费 compact、可追溯的 Learning Signal，不复制完整 transcript，也不把反思文本直接激活为新能力。

## 12. 权限、Workspace Trust 与 Sandbox

权限行为是 `allow | deny | ask`。外部可选模式包括 default、plan、acceptEdits、dontAsk、bypassPermissions，内部还可能有 auto/bubble：[permissions.ts](/Users/my/harness/claude-code-rev/src/types/permissions.ts:16)。
...
关键实现见：[permissions.ts](/Users/my/harness/claude-code-rev/src/utils/permissions/permissions.ts:1158)。后台/无 UI Agent 无法弹权限框时，会先让 PermissionRequest Hook 决策；仍无决定则自动 deny，而不是静默 allow：[permissions.ts](/Users/my/harness/claude-code-rev/src/utils/permissions/permissions.ts:929)。

Bash 安全比简单命令 allowlist 深得多。主路径优先使用 tree-sitter AST；若命令含 substitution、expansion、control flow 或 parser differential，无法静态证明安全则 ask：[bashPermissions.ts](/Users/my/harness/claude-code-rev/src/tools/BashTool/bashPermissions.ts:1663)。后续还包括：
...
- `CLAUDE_CODE_DISABLE_COMMAND_INJECTION_CHECK` 可关闭 AST 注入检查：[bashPermissions.ts](/Users/my/harness/claude-code-rev/src/tools/BashTool/bashPermissions.ts:1678)。
- OS sandbox 默认 `enabled ?? false`：[sandbox-adapter.ts](/Users/my/harness/claude-code-rev/src/utils/sandbox/sandbox-adapter.ts:459)。
- `allowUnsandboxedCommands` 默认 true：[sandbox-adapter.ts](/Users/my/harness/claude-code-rev/src/utils/sandbox/sandbox-adapter.ts:474)。
- sandbox 初始化失败默认记录日志后降级，而不是无条件 fail closed：[sandbox-adapter.ts](/Users/my/harness/claude-code-rev/src/utils/sandbox/sandbox-adapter.ts:782)。
- `--print` 跳过 trust，并应用可能来自不可信来源的完整环境变量；代码只通过帮助文本警告调用者目录必须可信：[main.tsx](/Users/my/harness/claude-code-rev/src/main.tsx:2590)。
- 非交互模式的 Hook、LSP 和其他执行路径把 trust 视为隐式成立。

因此安全机制可以作为“多层防御案例库”，不能作为“默认安全的无人值守执行器”照搬。oh-my-dsh 必须继续使用 DSH 原生 Approval、Permission Preset、FS/Shell 和 Sandbox；merge、release、生产部署、secret、付费操作和不可逆外部动作仍是 Protected Action，Evolution Loop 不得绕过。
...
- 工具池在合并后稳定排序，注释明确提到 prompt-cache stability：[tools.ts](/Users/my/harness/claude-code-rev/src/tools.ts:329)。
- 被 deny 的工具在模型看到 schema 前过滤，避免无效 schema 进入前缀。
- deferred tools 只在需要时加载；`alwaysLoad` 只留给首轮必须可见的能力：[Tool.ts](/Users/my/harness/claude-code-rev/src/Tool.ts:439)。
- Tool observer 输入的回填不修改原始 API-bound input，明确保护 prompt cache：[Tool.ts](/Users/my/harness/claude-code-rev/src/Tool.ts:474)。
...
oh-my-dsh 的 Cache Contract 比复制这些局部技巧更重要：同一 DSH Session 固定 Capability Generation；模型可见工具名、schema、顺序和稳定指令不因后台学习变化；动态状态用现有稳定工具按需读取；无变化 Hook 真正 no-op；Promotion 只影响后续 Session；缓存效果以完整 composition 测量。

## 14. 对长时间自治的真实评价

Claude Code Rev 已具备若干“长时间继续运行”的零件：

- max turns、美元预算和 task budget：[main.tsx](/Users/my/harness/claude-code-rev/src/main.tsx:2844)；
...
oh-my-dsh 应依照既定范围补足用户结果，而不是建立平行 Runtime：
...
## 17. 对 DSH / oh-my-dsh 的可借鉴点

以下是行为和原则层面的借鉴，不是源码移植建议。

| Claude Code Rev 经验 | DSH / oh-my-dsh 对应做法 |
|---|---|
...
- 不把 conversation JSONL 变成 oh-my-dsh 的第二套 Workflow 数据库。
...
简言之：**学它的控制面和生命周期，不信任恢复代码的精确性；借它延长 Agent 的运行，用 DSH 原生接缝和 oh-my-dsh 的证据闭环超越它的进化上限。**
