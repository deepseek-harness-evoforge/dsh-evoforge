...
同时，读取自身 `dist`/Typert 产物的 `dsh-doctor`、`dsh-evolve`、`dsh-gateway` 测试各自声明 `pretest` 构建；根级递归测试因此不依赖上一次本地构建遗留物。
...
- `dsh-doctor`、`dsh-evolve`、`dsh-gateway` 在本轮分别由包级 `pretest` 构建后，相关 clean-package 契约测试通过；`check:ci` 现在也固定这三类产物读取包及其余带客户端产物包必须拥有构建前置。
