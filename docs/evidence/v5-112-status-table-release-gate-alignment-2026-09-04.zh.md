...
在能力矩阵前增加权威状态口径，并将 Web Control Plane、Telegram 单私聊和 Hermes paired benchmark 的状态从
`implemented` 更正为 `partial`，双真实 Provider 保持 `not-run`、真实飞书 AS-2 保持 `partial`。本地实现、静态
合同和 fixture 不能覆盖 `release-gates.json` 的真实验收状态；发布声明必须服从最新门禁和同任务/同模型/同权限/
同预算证据。README、发布文档和当前限制已经使用同一口径，未删除历史证据，也未把失败改成通过。
