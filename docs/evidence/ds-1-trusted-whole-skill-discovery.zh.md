...
whole-Skill 隔离、准入与 release-authority 边界。durable Gap 与隔离 identity 的跨 Goal 聚合见
[V4-3](v4-3-cross-goal-gap-demand.zh.md)；聚类同样没有生成或发布权。
...
既有 Shadow 内核现在还能接收一个外部固定 Candidate：不调用 proposer，不依赖模型路由，把候选与
