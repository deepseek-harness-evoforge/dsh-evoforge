...
6. native Jobs scheduler 只在持久化结果为 `pass` 时回调既有 deterministic admission。其余状态保持
   quarantined；模块没有安装、激活、发布或晋升接口。
...
- `research-skill-holdout.test.ts` 覆盖精确逐锚通过、宿主派生 fail/inconclusive、非法覆盖失败关闭、作者/评估者身份冲突、预算延后恢复、不确定调用不盲重试、可执行内容拒绝、根隔离和仅 pass 下传。
