...
- 真实 Chrome 完成 Inspect → 填 note → 联合确认 → Cancel → 再确认 → durable Done，完整预算/队列布局无错位，控制台错误为 `0`；
- 64 轮真实 Agent 请求逐轮 byte-equivalent；PA-1 为 `125 passed / 1 skipped`；全仓为 `263 passed / 3 skipped`。
...
全量检查第一次遇到既有并行 clean 竞态：Telegram cache-composition 在其 `dist/index.mjs` 被并行构建清理时启动。独立 `dsh-telegram build && test` 为 `37/37`，随后完整 `pnpm check` 通过；P1.17 未修改 Telegram。该记录不把竞态误写成 P1.17 产品能力或上游修复。
