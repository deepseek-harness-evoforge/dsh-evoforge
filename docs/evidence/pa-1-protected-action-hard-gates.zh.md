...
本地 macOS 2026-08-17 结果：`166 passed | 1 skipped`；唯一 skip 是需要显式
...
| 进程常驻未经部署批准 | `dsh-resident/deployment.test.ts` 验证 apply/remove 缺少逐次 `--confirm-deployment` 时立即失败，既不创建 unit 目录也不调用 OS manager | 安装、启动、停止和删除用户级服务 |
