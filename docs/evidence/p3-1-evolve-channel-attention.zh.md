...
| 跨重启去重 | notice 与普通 turn 共用 Gateway 持久 delivery journal；终态重载后仍复用同一条记录 |
...
4. Suite 内部只适配 Telegram/Feishu 两个具体 route service；DSH Gateway 共享入口身份、native Agent 绑定和普通文本 outbound，但不等于公共通知 SPI 或巨型业务网关。
5. 自动提醒只减少轮询，不代表 Candidate 正确，也不会改变人工保护动作边界。

## 声明口径

可以声明：P3.1 Telegram/飞书最小注意力闭环已经实现，且没有破坏 DSH KV Cache 优势。
