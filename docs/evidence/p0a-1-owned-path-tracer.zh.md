...
6. provider 报告的 input/output token 超出 case pack 上限时保存实际用量并返回 `2`；
7. 模型 API key 只进入 Authorization header，不出现在报告、证据、stdout 或 stderr；
8. 正常路径下 active Skill 内容不变，越权文件不存在，成功路径只产生 `evidence/proposal.json` 与 `report.json`。
