...
- failed-Outcome Canary Host/Jobs 已由 [V4.46](v4-46-existing-skill-failed-outcome-canary.zh.md) 补齐，Control/Web 与证据驱动精确回滚已由 [V4.47](v4-47-existing-skill-canary-control-rollback.zh.md) 补齐；最终包 Canary/rollback 恢复、两套独立真实 provider、真实飞书 exact route、Hermes paired benchmark 与长期误晋升/负迁移/误回滚数据仍阻止 tag 和完成声明。
