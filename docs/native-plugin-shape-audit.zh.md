...
| `dsh-telegram` | 是；Bundle row `evoforge-telegram`，hard inject DSH Gateway | 无 webhook server/daemon/第二 Session；不接收 agent/chat/user 配置；外部已发送消息不是本地权威 | Gateway 负责 Workspace/Session/Agent/Command/入口幂等；Adapter 只持有 Telegram 长轮询、Approval UI、outbound delivery journal，均由 effect/fiber 释放 | 官方 add/remove；默认 disabled，profile patch 只命名 exact routeId/token env | 已迁为首个 Gateway Adapter；不扩展为 gateway |
...
| `dsh-gateway` | 是；Bundle row `evoforge-gateway`，提供 Host Gateway service | 无网络 server、平台轮询、第二 Session/Agent/Approval/Goal 或权限系统 | WorkspaceRegistry、Agent/Session persistence、Agent presets、Commands、Storage Domain；exact endpoint 与 uncertain ingress fail closed | 官方 add/remove；默认 disabled，完整静态 route 配置 | 作为 Telegram/飞书共用的最小 Host seam；平台协议与 outbound 留在 Adapter |
| `dsh-feishu` | 是；同包 Bundle row `evoforge-feishu` + 官方 `dsh.client` web module；Host hard inject Commands、DSH Gateway、StorageDomain、WorkspaceRegistry | 无 Webhook server、daemon、独立网站、第二 Session/Goal/权限系统；官方 SDK 是平台协议依赖 | Gateway 负责正常身份/Workspace/Agent/Command；Adapter 的 WebSocket、Approval 卡片与持久 outbound worker 由 effect 释放；setup-only `/feishu-pair` 只生成待审查静态 route，不 dispatch Agent；Client 只在当前 Session 存在该 Command 时注册原生侧栏动作 | 一个 tarball 官方 add/remove；默认 disabled，exact routeIds 与凭据 env；pairing mode 必须显式启用且 routeIds 为空 | 第二个 Gateway Adapter；明确 429 有界重试，模糊发送为 uncertain；同一包内的 DSH Web 向导复用 Command authority，消除手工查平台 ID 与拼命令的新手负担 |
