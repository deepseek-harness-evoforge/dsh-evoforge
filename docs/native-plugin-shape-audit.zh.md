...
| `dsh-gateway` | 是；Bundle row `evoforge-gateway` + 同包官方 `dsh.client` web module，提供 Host Gateway service 和只读健康投影 | 无网络 server、平台轮询、第二 Session/Agent/Approval/Goal 或权限系统；Client 无模型、平台或写操作 | WorkspaceRegistry、Agent/Session persistence、Agent presets、Commands、Storage Domain；exact ingress、公共 outbound intent/journal 与脱敏 transport observation 在模糊 effect 边界 fail closed；生成式 Typert Remote 只读既有 `healthSnapshot()` | 官方 add/remove；默认 disabled，完整静态 route 配置；Client 与 Host 同包卸载 | 作为 Telegram/飞书共用的最小 Host seam；统一普通文本幂等、按 account 串行、明确限流重试和 exact-route 健康；DSH Web 失败时清空旧快照；平台协议/发送/重连/卡片留在 Adapter |
| `dsh-feishu` | 是；同包 Bundle row `evoforge-feishu` + 官方 `dsh.client` web module；Host hard inject Commands、DSH Gateway、WorkspaceRegistry | 无 Webhook server、daemon、独立网站、第二 Session/Goal/权限系统；官方 SDK 是平台协议依赖 | Gateway 负责正常身份/Workspace/Agent/Command 与普通文本 outbound；Adapter 的 WebSocket/HTTP、实际发送和 Approval 卡片由 effect 释放；setup-only `/feishu-pair` 只生成待审查静态 route，不 dispatch Agent；Client 只在当前 Session 存在该 Command 时注册原生侧栏动作 | 一个 tarball 官方 add/remove；默认 disabled，exact routeIds 与凭据 env；pairing mode 必须显式启用且 routeIds 为空 | 第二个 Gateway Adapter；私有 Delivery Store/worker 已删除；429/uncertain 归约到 Gateway 公共结果；同包 DSH Web 向导复用 Command authority |
