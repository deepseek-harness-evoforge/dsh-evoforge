...
| `dsh-gateway` | 是；Bundle row `evoforge-gateway`，提供 Host Gateway service | 无网络 server、平台轮询、第二 Session/Agent/Approval/Goal 或权限系统 | WorkspaceRegistry、Agent/Session persistence、Agent presets、Commands、Storage Domain；exact ingress 与公共 outbound intent/journal 在模糊 effect 边界 fail closed | 官方 add/remove；默认 disabled，完整静态 route 配置 | 作为 Telegram/飞书共用的最小 Host seam；统一普通文本幂等、按 account 串行、明确限流重试和健康，但平台协议/发送/卡片留在 Adapter |
