...
| `dsh-feishu` | 是；Bundle row `evoforge-feishu`，hard inject Commands、Channel Router、StorageDomain、WorkspaceRegistry | 无 Webhook server、daemon、第二 Session/Goal/权限系统；官方 SDK 是平台协议依赖 | Router 负责正常身份/Workspace/Agent/Command；Adapter 的 WebSocket、Approval 卡片与持久 outbound worker 由 effect 释放；setup-only `/feishu-pair` 只生成待审查静态 route，不 dispatch Agent | 官方 add/remove；默认 disabled，exact routeIds 与凭据 env；pairing mode 必须显式启用且 routeIds 为空 | 第二个 Router Adapter；明确 429 有界重试，模糊发送为 uncertain；两分钟一次性配对消除手工查平台 ID 的新手负担 |
