...
| `dsh-telegram` | 是；Bundle row `evoforge-telegram`，hard inject Channel Router | 无 webhook server/daemon/第二 Session；不接收 agent/chat/user 配置；外部已发送消息不是本地权威 | Router 负责 Workspace/Session/Agent/Command/入口幂等；Adapter 只持有 Telegram 长轮询、Approval UI、outbound delivery journal，均由 effect/fiber 释放 | 官方 add/remove；默认 disabled，profile patch 只命名 exact routeId/token env | 已迁为首个 Router Adapter；不扩展为 gateway |
...
| `dsh-feishu` | 是；同包 Bundle row `evoforge-feishu` + 官方 `dsh.client` web module；Host hard inject Commands、Channel Router、StorageDomain、WorkspaceRegistry | 无 Webhook server、daemon、独立网站、第二 Session/Goal/权限系统；官方 SDK 是平台协议依赖 | Router 负责正常身份/Workspace/Agent/Command；Adapter 的 WebSocket、Approval 卡片与持久 outbound worker 由 effect 释放；setup-only `/feishu-pair` 只生成待审查静态 route，不 dispatch Agent；Client 只在当前 Session 存在该 Command 时注册原生侧栏动作 | 一个 tarball 官方 add/remove；默认 disabled，exact routeIds 与凭据 env；pairing mode 必须显式启用且 routeIds 为空 | 第二个 Router Adapter；明确 429 有界重试，模糊发送为 uncertain；同一包内的 DSH Web 向导复用 Command authority，消除手工查平台 ID 与拼命令的新手负担 |
