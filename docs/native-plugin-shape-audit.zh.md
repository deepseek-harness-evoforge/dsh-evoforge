...
| `dsh-gateway` | 是；Bundle row `evoforge-gateway` + 同包官方 `dsh.client` web module，提供 resident Host Gateway service、健康投影和配对批准/撤销 | 无网络 server、平台轮询、第二 Session/Agent/Approval/Goal 或权限系统；Client 无模型或平台调用 | WorkspaceRegistry、Agent/Session persistence、Agent presets、Commands、Storage Domain；未知 DM 在 Agent 前形成 hashed pending request，Web Remote 把 code 原子批准到当前 live native Session，或把动态 grant 原子转为 revoked tombstone；静态 route、活动 effect、Session 删除 fail closed；exact ingress、公共 outbound intent/journal 与脱敏 transport observation 在模糊 effect 边界 fail closed | 官方 add/remove；**安装即启用**，默认 `routes: []` 不连接平台；静态 routes 或 Gateway-owned future route grant；Client 与 Host 同包卸载 | Telegram/飞书共用的最小 Host seam；平台协议/发送/重连/卡片留在 Adapter；配对不用 Session Command、profile rewrite 或重启 |
