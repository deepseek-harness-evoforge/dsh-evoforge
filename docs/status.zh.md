...
最新 capability-absent baseline 增量通过根级 `pnpm check`；其中 `dsh-evolve` 58 files passed、1 file skipped，
273 tests passed、2 skipped。Cache Contract 全通过，Doctor 原生合同 22/22，十一包 clean-profile
tarball add/dump/boot/真实 Session+Goal+Tool/dispose/remove/reboot/readback 1/1（25.96 秒）。
...
- 自我发现只允许从 DSH 内部 Goal、Gap、失败、纠正、结果、复用与保留证据学习；同 Goal retry 不计独立证据，任何 Opportunity/Candidate 都没有安装、激活或发布权；配置已不能预选内部 Candidate Skill，缺失 Skill baseline 也不再伪造占位 Skill，但当前仍缺治理包自主形成、新 Skill 内容寻址 Publisher/Retention、真实 provider 独立评估整链 outcome 和模型缺口质量；
