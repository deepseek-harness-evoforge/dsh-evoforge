...
| 显式负反馈学习入口 | `implemented`（P1.3） | [P1.3](evidence/p1-3-explicit-feedback-intake.zh.md)：真实 DSH Message Feedback、reference-only/可撤回 Storage、host-only 聚合、重启/容量、零模型请求 | 将新颖反馈安全变成可重放 Case；真实用户样本与候选改善率 |
...
- 不能把 P1.1–P1.3/P2D.1 当作已验证的完整 bounded autonomy；已有反馈 intake 与反事实自动回滚，但尚不能把新颖反馈自动变成可重放 Case，也无真实任务长期误晋升、误回滚和 review-rate 数据；
