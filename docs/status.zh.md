...
| Git Skill 晋升与回滚内核 | `implemented`（host service + P0C.1 command） | exact Git gate、future-session pointer、live Session 不漂移；[P0C.1](evidence/p0c-1-human-release-command.zh.md) status/promote/rollback | Candidate review projection、P1 自动晋升与真实 canary 数据 |
| 异步人工复核 | `planned` | P0C.1 已提供 release command，但尚无 Candidate inbox | claim/diff/case/cost projection、review 与不阻塞原会话测试 |
...
- 通过 host-only `/evolve status | promote <id> | rollback` 查看或人工切换 future-session Generation，不产生模型请求；
- 在真实 DSH Agent 上让 root/resume/child 固定各自 Generation；pin 或 Git 完整性失败时原生会话继续；
- 审查报告 Schema 的实际 JSON 输出。

## 当前不能做什么

- 不能把 P0C.1 release command 当作完整人工复核产品；它尚不展示 Candidate claim/diff/case/cost，也没有 review inbox 或自动晋升政策；
...
P0B 的本地实现门已通过；P0C.1 人工 release command 已实现且保持零模型调用、future-session-only。下一纵切是 durable Candidate/review projection，而不是新的控制平台；生产多日 soak 继续作为发布证据积累，不能被短时自动化测试替代。
