...
EvoForge 已有大量 `implemented` 能力，但 v0.1 **未完成**。所有已提交成果都在权威 `main`；`dsh-gateway` 已直接替换 `dsh-channel-router`，不保留兼容转发包，并通过 Gateway/Telegram/飞书/Evolve Attention、全仓类型/构建和十一包 clean-profile 回归。公共 outbound、限流和权威健康投影尚未收敛到 Gateway，旧能力获取/研究 Candidate 领域残留也尚未删除完。当前自我发现纵切是自然语言 Goal → Host 复核/持久 Capability Gap → DSH 内部跨 Goal Skill Opportunity → 原生 Job 生成 instruction-only whole-Skill v1 → inactive/quarantined Candidate；它仍只覆盖重复 Goal-linked Gap，尚未完整吸收纠正、真实结果、返工、成本、复用、Retention 和回滚证据。真实飞书 exact 消息、内部 Candidate 独立评测、真实 provider、Hermes paired 和长期数据均未达标。
