...
| Delivery Outcome 第二消费者 | `implemented`（P2D.1 + P1.21） | [P2D.1](evidence/p2d-1-delivery-outcome-signal.zh.md)：真实 ToolRuntime/Storage、异步不阻塞、幂等/容量/重启、host-only 聚合、零模型请求；[P1.21](evidence/p1-21-parent-generation-outcome-comparison.zh.md)增加 active-vs-parent 描述性对照 | 同任务分布的真实开发样本；P1.2 已补反事实 canary，观察计数和单次失败仍不能直接回滚 |
...
| 父版本交付结果对照 | `implemented`（P1.21） | [P1.21 证据](evidence/p1-21-parent-generation-outcome-comparison.zh.md)：既有 bounded outcome 单次聚合、active vs exact parent/native、Commands/Web 因果免责声明、Storage restart、真实 DSH/Chrome、零模型表面 | 同任务分布的真实 active/parent 样本、长期置信区间与用户是否据此减少错误回滚 |
| 单机常驻和崩溃恢复 | `implemented`（P0B + LC-1 + LC-2） | 四个 release `SIGKILL` 边界；[P0B.2a](evidence/p0b-2a-durable-shadow-resume.zh.md)；[P0B.2b](evidence/p0b-2b-resident-shadow-supervisor.zh.md) native Jobs supervisor/关机恢复/重复扫描；[LC-1](evidence/lc-1-goal-continuity.zh.md) exact Session 原生 Goal 冷恢复；[LC-2](evidence/lc-2-resident-service.zh.md) OS 进程拉起 | Linux 真机 crash/login、生产多日 soak、真实磁盘耗尽与更多机器数据 |
...
| `dsh-github-review` Draft PR 审查返修 | `implemented`（P3.2） | [P3.2 证据](evidence/p3-2-github-review-followup.zh.md)：canonical delivery watch、exact-head allowlist human、bounded untrusted follow-up、崩溃窗口去重、有界 Storage、真实 DSH、request parity、tarball add/boot/remove、GitHub 授权与公开 API 冒烟 | 真实 `CHANGES_REQUESTED`→Agent 返修→verified push，rate-limit/公网故障、多日 resident、陌生安装数据 |
...
| `dsh-evolve-telegram` 进化注意力桥 | `implemented`（P3.1） | [P3.1 证据](evidence/p3-1-evolve-telegram-attention.zh.md)：真实 supervisor signal、权威 overview 投影、durable notice 去重与 crash uncertainty、三组 request byte parity、tarball add/boot/remove | 真实 Bot/移动端/公网故障、多日 soak、陌生安装与 Hermes paired benchmark |
| `dsh-goal-continuity` 原生 Goal 冷恢复 | `implemented`（LC-1） | [LC-1 证据](evidence/lc-1-goal-continuity.zh.md)：默认 disabled、exact allowlist、真实 JSONL 两进程恢复、`SIGKILL`、原生轮次上限、人工恢复 cache surface 等价、tarball add/boot/remove | 生产多日 soak、陌生安装、崩溃恢复时延与有意重启策略数据 |
| `dsh-resident` 用户级进程常驻 | `implemented`（LC-2） | [LC-2 证据](evidence/lc-2-resident-service.zh.md)：只读 plan、Protected apply/remove、absolute no-shell unit、真实 macOS `SIGKILL`→新 PID→删除、systemd 生命周期协议、packed CLI | Linux 真 user manager crash/login、陌生安装、日志增长与生产多日恢复率/时延 |
...
| Web 控制面 | `implemented`（P0C.6 + UI-1 + P1.20–P1.21） | 可删除 Bundle、生成式 Remote、全局侧栏入口；固定 DSH tarball 装配与真实浏览器 pause → 进程重启仍暂停 → resume；真实 Chrome 已完成 claim/evidence/limitations/diff → inactive approval、automatic review window/open/eligible/stale 刷新与 active/parent outcome 对照，[证据](evidence/p1-21-parent-generation-outcome-comparison.zh.md) | 陌生用户可用性数据、分页/图形 diff；实时推送仅在证据显示需要时考虑 |
...
- active Generation 存在时，在 Commands/Web 并列查看其与 exact parent/native DSH 的交付三态计数；该对照固定带因果免责声明，不自动晋升或回滚；
...
- 启用 `dsh-github-review` 后，让 allowlist 人类对当前 Draft PR exact head 的 `CHANGES_REQUESTED`
  作为有界、不可信数据回到原 Session，继续同一个 Goal；无 actionable review 时 0 模型调用和 token。
...
- 同时启用 `dsh-evolve-telegram` 后，在 Candidate 或 Evaluator Draft 需要处理时收到一次有界提醒；
  原会话继续，用户复制既有 `/evolve` 命令查看和决定，消息本身不构成审批。
- 显式配置 exact 持久 Session allowlist 后，用 `dsh-goal-continuity` 在进程重启时自动 rearm
  仍 active 且有剩余轮次的原生 Goal；paused/blocked/complete、非授权 Session 与新建 Session 不变。
- 用 `dsh-resident plan` 查看一个 exact profile 的完整用户级 unit；明确确认后安装并启动，用 `status`
  区分 manager registration、当前 active 与 unit file，用 `remove` 停止并删除 unit。
...
- 不能把 P1.1–P1.21/P2D.1 当作已验证的完整 bounded autonomy；已有反馈 intake、私有 Case Draft、既有 evaluator 下的 opt-in 自动 Shadow、全新失败的 opt-in 自动 inactive Evaluator Draft、人工 Qualify-and-Shadow、每 Skill 单未决门、模糊审查有界过期及窗口可见性、父版本观察计数、持久日预算、proposer 前校准、单 prior Target 自动 Retention 与反事实自动回滚，但没有默认后台 author 或自动 evaluator qualification，也无真实 provider、同任务分布的长期误晋升、误回滚、遗忘和 review-rate 数据；
...
- 不能把 `dsh-github-review` 当作 Review 平台或生产级自治返修：它只看一个 Agent + repository 的
  当前 Draft PR，尚无真实 reviewer 返修和多日 soak，不支持 Enterprise/fork/webhook/App/自动 merge；
...
- 不能把 `dsh-telegram` 当作生产消息平台或通用 Gateway：尚未跑真实 Bot 多日 soak、陌生用户安装
  ...
  也不能撤回已经发送的消息；
- 不能把 `dsh-evolve-telegram` 当作通用通知平台或审批代理：它只复用一个 exact Telegram route，
  没有摘要、升级通知、多收件人、其他渠道或 inline protected action；
- 不能把 `dsh-goal-continuity` 当作进程管理器、高可用或通用任务调度器：它不启动 DSH、不扫描任意
  冷 Session、不重试同进程 provider 故障，静态授权也不能区分崩溃与有意重启；
- 不能把 `dsh-resident` 当作高可用、秘密管理、日志平台或 DSH 状态修复器：它只管理一个用户级
  OS unit；Linux 真机 crash/login 和多日证据尚未完成，Windows 不支持；
- 不能作为生产依赖安装。

P0B 的本地实现门已通过；LC-1 用一个默认关闭的 exact Session policy 接通原生 Goal 冷恢复，LC-2 再以 4 个动作的 OS adapter 拉起 exact DSH profile；两者都不新增 Mission、任务库或模型表面。P0C.1–P0C.6 已形成含 exact bounded diff、protected-effect 词法提示、零模型调用和真实 DSH Web 入口的人工闭环；P1.1 已增加默认关闭、可解释、崩溃可恢复的最窄自动晋升。P2A–P2C.3 已把真实 Git/local checks/Draft PR/exact-head remote checks 与有界零模型等待绑定到一个原生 Goal 的受验证完成路径；P3.2 再让 allowlist 人类的 exact-head 修改要求以不可信尾部消息回到同一 Goal，而没有新建 Mission、Review 平台或模型常驻面；P2D.1 已让 Evolve 成为不阻塞、零模型表面的第二消费者；P1.2 已用原 Case Pack 和 exact Git parent/Candidate 实现可归因自动回滚；P1.3/P1.4 已把明确纠正保存为私有、未评分 Case Draft；P1.5 已让 exact 草稿只引导 proposer，由既有可信 Case Pack 独立裁判；P1.6 已把 evaluator 方向校准提前到 proposer 之前；P1.7 已提供不进入 runtime surface 的显式 authoring Skill；P1.8/P1.14 分别提供逐次显式或部署策略授权的静态 Target Shadow，P1.15 用 crash-safe 日预算限制自动尝试；P1.9 把全新失败收敛为私有 inactive Evaluator Draft，并把 exact-hash 人工语义审查与真实 DSH sealed qualification 分开；P1.16 可按静态策略自动生成该 inactive Draft；P1.17 允许人工在一次明确付费披露中把 qualification 与成功后的 Shadow 合并；P1.18 又在预算前复用 Draft/Shadow/Review 事实，避免同一 Skill 的重复自动请求；P1.19 只对过期的 automatic `review` 复用 durable rejection，防止无人审批永久停学；P1.20 再把 exact 窗口与唯一触发条件投影给 Commands/Web，不新建 timer 或状态；P1.21 只用既有 outcome 显示 active/parent 观察计数并禁止因果推断。上述能力不自动 qualification，也不会把模糊候选晋升；原会话均不等待。AS-1 首片由 `dsh-telegram` 实现，P3.1 再用 `dsh-evolve-telegram` 补齐非阻塞注意力，但都仍只到自动化 `implemented`。下一步应收集独立陌生作者、真实 provider/任务 outcome、纠正改善率、真实 reviewer 返修、Goal/进程恢复时延、Linux 真机与真实 Bot/Hermes paired 数据，而不是扩大成新 Memory/Signal/Workflow/Gateway/Supervisor/Review 平台。P0C 陌生用户可用性和生产多日 soak 继续作为证据积累，不能被短时自动化测试替代。
