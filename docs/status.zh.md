...
| Evidence-driven Evolution P0A–P1.21 + internal Skill Opportunity | `implemented` | paired Trial、Workspace-owned Generation/feedback/review/budget、Session pin、Retention、canary；自然 Goal→Host 复核/持久 Gap；至少两个独立 Goal 形成 Skill Opportunity；[V4.11](evidence/v4-11-durable-feedback-skill-attribution.zh.md) private Feedback Draft 不接受用户选择 Skill，而从 durable turn 的唯一 invocation 推导并核对 pinned Generation，Shadow/Evaluator 静态目标不匹配时在 Git/Job/模型前拒绝；同 Session 唯一 Skill 纠正和 stable Goal identity 跨 revision、时间/revision 单调的唯一 Skill Outcome 作为 `causalClaim: none` 上下文并可跨 Storage restart 恢复；Outcome 可选保存 exact Goal turn 的官方 token/cache/latency projection；Host 权威 rollup/最近证据经 Remote 进入 Web，真实浏览器验证刷新/断连/恢复且重放不重复；Workspace-only policy 无 Skill 预配置；内部证据 author→canonical whole-Skill v1 quarantine；Candidate Repository/Admission/Lineage/Shadow 只接受内部契约；Web Gap→Opportunity→Candidate；64 轮稳定 Tool 与原生其余请求 parity | correction/outcome 到 Opportunity 的 exact invocation 因果链接，以及 rework/currency-cost/reuse/retention/negative-transfer/rollback 完整归因；内部 Candidate 仍依赖预配置 Skill/Case Pack，尚缺证据派生的独立 evaluation envelope、final-test/Shadow/Retention 真实 provider 整链路、路由质量及长期误晋升/回滚数据 |
