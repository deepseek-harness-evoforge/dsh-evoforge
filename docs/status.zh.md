...
> **最新增量（V5.198）**：修复常驻 `dsh-gateway` 同一 Host 并发启动的生命周期竞态：并发 `start()` 现在共享唯一 Promise，避免重复恢复 journal、注册 session listener 或重复路由校验；启动失败回收、停止态 fail-closed 和 Typert 契约保持不变。重新生成 pinned DSH Typert digest 后，Gateway 构建/产物校验及 `44/44` 测试通过（新增并发启动回归）。详见 [V5.198 证据](evidence/v5-198-gateway-concurrent-start-2026-09-04.zh.md)。

> **最新增量（V5.199）**：在 V5.198 修复后重新 fetch/audit 最新 canonical DSH 并执行根级全量检查，权威 `CHECK_RC=0`；Evolution `309/309`、Gateway `44/44`、Feishu `56/56`、Telegram `38/38`、Evolve Web `27/27`、Control Center `27/27`、Doctor `40/40`，其余 Bundle、clean-profile、Typert、兼容性与合同门均通过。最新 DSH 根构建缺陷继续按上游事实分类，真实渠道/Provider/Hermes/长期/npm 门禁仍未通过。详见 [V5.199 证据](evidence/v5-199-full-check-after-gateway-start-2026-09-04.zh.md)。
...
> **最新增量（V5.195）**：Telegram 静态与 pairing Adapter 现在具备单次启动闸门和完整失败清理；teardown 的健康上报或单个 disposer 失败不会阻断其余资源回收。最新 DSH `d347e703…` 安装通过、根构建缺陷仍按上游分类，EvoForge 根检查 `CHECK_RC=0`；Telegram `38/38`、Feishu `56/56`、Gateway `43/43`、Evolution `309/309`。详见 [V5.195 证据](evidence/v5-195-telegram-startup-boundary-2026-09-04.zh.md)。发布门禁不变。
