...
Gateway 已统一 Telegram/飞书普通文本 outbound、幂等、限流、uncertain 恢复、transport observation 和健康快照；Web 已做真实浏览器读取/刷新/断连恢复。现有 Skill 的同版本跨 Goal 精确纠正先形成 investigation；V4.33 在原生调用边界封存完整内容寻址 baseline Bundle，V4.34 再把每个 exact correction 的 Session/invocation reference 逐一重验并要求全部指向同一 Bundle，只有这样才获得无发布权的 existing-Skill authoring 资格。protected author、Candidate、paired evaluation、完整返工/成本/复用/Retention/负迁移/回滚归因、真实飞书 exact 消息、真实 provider、Hermes paired 和长期数据仍未达标。
