...
EvoForge 已有大量 `implemented` 能力，但 v0.1 **未完成**。所有已提交成果都在权威 `main`；`dsh-gateway` 已直接替换 `dsh-channel-router`，不保留兼容转发包，并通过 Gateway/Telegram/飞书/Evolve Attention、全仓类型/构建和十一包 clean-profile 回归。旧能力获取/运行时 research Candidate 的活动源码、依赖、持久化变体和 Web 投影已删除，当前 Candidate seam 只接受内部 Skill Opportunity 生成的 canonical text bundle。Gateway 自有 route/Session/ingress 健康已可脱敏权威投影；公共 outbound、跨 Adapter 限流、transport 健康聚合和统一 Web 展示尚未收敛。当前 Opportunity 资格仍只来自重复 Goal-linked Gap，并已保守关联同 Session 的明确纠正引用，以及同一稳定 Goal id 下不早于 Gap、revision 不倒退的真实交付结果；这些上下文固定无因果、无资格影响。Delivery Outcome 已可附带 exact Goal-owned turn 的官方 token/cache/latency facts，Host 权威聚合和有界最近证据也已进入 DSH Web，并通过真实 Host 的刷新、断连、同 profile 重启恢复与幂等重放；但货币成本、返工、复用、Retention、负迁移和回滚完整归因仍缺。真实飞书 exact 消息、内部 Candidate 独立评测整链、真实 provider、Hermes paired 和长期数据均未达标。
...
| Evidence-driven Evolution P0A–P1.21 + internal Skill Opportunity | `implemented` | paired Trial、Workspace-owned Generation/feedback/review/budget、Session pin、Retention、canary；自然 Goal→Host 复核/持久 Gap；至少两个独立 Goal 形成 Skill Opportunity；同 Session 唯一 Skill 纠正和 stable Goal identity 跨 revision、时间/revision 单调的唯一 Skill Outcome 作为 `causalClaim: none` 上下文并可跨 Storage restart 恢复；Outcome 可选保存 exact Goal turn 的官方 token/cache/latency projection；Host 权威 rollup/最近证据经 Remote 进入 Web，真实浏览器验证刷新/断连/恢复且重放不重复；Workspace-only policy 无 Skill 预配置；内部证据 author→canonical whole-Skill v1 quarantine；Candidate Repository/Admission/Lineage/Shadow 只接受内部契约；Web Gap→Opportunity→Candidate；64 轮稳定 Tool 与原生其余请求 parity | correction/outcome 的 exact invocation 因果链接，以及 rework/currency-cost/reuse/retention/negative-transfer/rollback 完整归因；内部 Candidate 的独立 final-test/Shadow/Retention 真实 provider 整链路、路由质量及长期误晋升/回滚数据 |
...
| Workspace DSH Gateway | `implemented` | `dsh-gateway` 直接替换旧包且无兼容层；exact endpoint deny-by-default；原生 Workspace/Session/Agent create/resume；持久幂等/uncertain 状态机；Gateway 自有 route/live Session/ingress 脱敏快照；Gateway 10、Telegram 39、飞书 41、Attention 18 项测试及十一包 clean-profile 总装通过 | 公共 outbound、跨 Adapter 限流、transport 健康聚合、统一 Web 展示、exact 飞书 chat/user 消息闭环 |
