...
| 全新失败 evaluator authoring | `implemented` workflow（P1.7） | [P1.7](evidence/p1-7-evaluator-authoring-skill.zh.md)：显式 user-invoked Skill、独立分区/negative controls、真实 assembled calibration；一个新“进度即停”失败完成 red→green→bounded Shadow→可回滚 Skill 晋升；无 runtime surface | 独立陌生作者可用性、真实 provider/任务 outcome；仍不自动生成 evaluator |
...
| Automatic Evolution Budget | `implemented`（P1.15，随 P1.14 默认关闭） | [P1.15 证据](evidence/p1-15-automatic-evolution-budget.zh.md)：每 Target 默认 1/日、启动前 `0600` 原子预留、幂等/重启/UTC rollover、损坏与 symlink fail closed、Commands/Web bounded 用量、真实 DSH 自动链路 | 真实 provider 单位成本、长期 cap 命中率、陌生用户配置与生产多日 soak |
...
| Web 控制面 | `implemented`（P0C.6 + UI-1 审查卡） | 可删除 Bundle、生成式 Remote、全局侧栏入口；固定 DSH tarball 装配与真实浏览器 pause → 进程重启仍暂停 → resume；真实 Chrome 已完成 claim/evidence/limitations/diff → inactive approval 闭环，[证据](evidence/ui-1-explainable-review.zh.md) | 陌生用户可用性数据、分页/图形 diff；实时推送仅在证据显示需要时考虑 |
...
- 为每个 Automatic Feedback Target 设置 `maxAttemptsPerUtcDay`；`/evolve status` 与 Web 可读当日 used/limit/remaining，额度耗尽只阻止自动入口，不阻塞原会话或显式人工 Shadow；
...
- 不能把 P1.1–P1.15/P2D.1 当作已验证的完整 bounded autonomy；已有反馈 intake、私有 Case Draft、既有 evaluator 下的 opt-in 自动 Shadow、持久日预算、proposer 前校准、显式/受审查的 Evaluator Draft、单 prior Target 自动 Retention 与反事实自动回滚，但还没有默认后台自动 evaluator author/qualification，也无真实 provider、真实任务长期误晋升、误回滚、遗忘和 review-rate 数据；
