...
- 演化概览不再把未验证草稿或读取失败显示为健康空闲；普通草稿明确没有直接启用入口。Web 包已部署并实际刷新验证，
  旧失败/得分和全部账本保留，见[概览状态验收](evidence/2026-09-18-evolution-summary-state.zh.md)。这不是学习效果证明。
...
9 月 18 日[Web 审批拒绝验收](evidence/2026-09-18-web-approval-denial.zh.md)通过单一真实切片：独立只读会话
写入失败后申请一次审批，点击拒绝后停止、目标文件不存在，刷新可恢复拒绝记录，原绑定会话权限未变。
等待审批时仍显示「深度求索中」和部分英文内部术语，已列为 P2；不宣称允许执行、飞书审批或审批中重启已验证。
