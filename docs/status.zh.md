...
| 显式负反馈学习入口 | `implemented`（P1.3） | [P1.3](evidence/p1-3-explicit-feedback-intake.zh.md)：真实 DSH Message Feedback、reference-only/可撤回 Storage、host-only 聚合、重启/容量、零模型请求 | P1.4 已提供显式私有 Draft；仍缺 scored Case、真实用户样本与候选改善率 |
| 私有 Feedback Case Draft | `implemented`（P1.4） | [P1.4](evidence/p1-4-private-feedback-case-draft.zh.md)：显式双重授权、exact feedback/Generation/Skill 归因、私有内容寻址落盘、幂等/撤回/歧义拒绝、零模型请求 | 为一个高频失败编译 deterministic reproduction/evaluator，形成有分数的 sealed Case |
...
- 不能把 P1.1–P1.4/P2D.1 当作已验证的完整 bounded autonomy；已有反馈 intake、私有 Case Draft 与反事实自动回滚，但尚不能把 Draft 编译成可重放、有评分的 Case，也无真实任务长期误晋升、误回滚和 review-rate 数据；
