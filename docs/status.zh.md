...
最新 content-addressed new-Skill Generation 增量通过根级 `pnpm check`；其中 `dsh-evolve` 58 files passed、
1 file skipped，279 tests passed、2 skipped。Cache Contract 全通过，Doctor 原生合同 22/22，十一包
clean-profile tarball add/dump/boot/真实 Session+Goal+Tool/dispose/remove/reboot/readback 1/1（28.78 秒）。
...
- 自我发现只允许从 DSH 内部 Goal、Gap、失败、纠正、结果、复用与保留证据学习；同 Goal retry 不计独立证据，任何 Opportunity/Candidate 自身都没有安装、激活或发布权；配置已不能预选内部 Candidate Skill，缺失 Skill baseline 不再伪造占位 Skill，review 后的新 Skill 内容寻址 Publisher/future Session/root rollback 已实现，但当前仍缺治理包自主形成、capability-absent Retention/canary、真实 provider 独立评估整链 outcome 和模型缺口质量；
