...
`dsh-evolve` 54 files/214 tests passed、1 file/1 test skipped；根级累计 443 tests passed、3 skipped。Cache Contract 全通过；Doctor 十一包
...
自主内部治理、不出现路线选择”；V4.28 已用 test-only exact-lineage fixture 从最终 tarball 重跑完整评测视图的真实浏览器 reload/断连/恢复；V4.29 又从最终 tarball 验证 promotion eligible/blocked、失败保留、恢复和卸载，fixture 均不进入发布包。
...
- 自我发现只允许从 DSH 内部 Goal、Gap、失败、纠正、结果与复用证据学习；同 Goal retry 不计独立证据，任何 Opportunity/Candidate/Retention/Canary verdict 自身都没有安装、激活或发布权；配置已不能预选 Candidate Skill，authoring/admission/holdout/Retention 样本在 Candidate 调用前隔离，Candidate v2/Lineage v3/Envelope v4-v5 显式绑定 seal，Shadow、Retention 与 failed-Outcome Canary 只消费 exact Candidate 且零 proposer，缺失 Skill baseline 不再伪造占位 Skill，review 后的新 Skill Publisher、future-Session Promotion Eligibility 与 exact evidence/human Rollback Gate 已实现；真实 provider assembled 评估、最终 tarball 浏览器 rollback、长期负迁移/误回滚率和模型缺口质量仍缺；
