...
| Evidence-driven Evolution P0A–P1.21 + internal Skill Opportunity | `implemented` | paired Trial、Workspace-owned Generation/feedback/review/budget、Session pin、Retention、canary；自然 Goal→Host 复核/持久 Gap；至少两个独立 Goal 形成 Skill Opportunity，至少四个后预密封 proposer-visible/admission/holdout；Candidate v2/Lineage v3 显式绑定 seal；Candidate-independent 治理作者只见各自 protected subset，零 proposer 校准并原子安装 Envelope v4；paid-call uncertain restart 不重试；Web 显示 readiness、保护边界和 seal 谱系；[V4.20](evidence/v4-20-exact-durable-feedback-attribution.zh.md) correction 只从 feedback 目标回答的唯一 durable Skill invocation 与 exact Goal id/revision 归因，并跨 Storage restart 恢复，弱 same-Session 猜测被拒绝；stable Goal identity 跨 revision、时间/revision 单调的唯一 Skill Outcome 仍作为 `causalClaim: none` 上下文；Outcome 可选保存 exact Goal turn 的官方 token/cache/latency projection；Workspace-only author/evaluation policies 均无 Skill 预配置；内部证据 author→canonical whole-Skill v1 quarantine；[V4.14](evidence/v4-14-capability-absent-baseline.zh.md) 以无目标 Skill 的真实 assembled DSH baseline 对比 exact Candidate；[V4.15](evidence/v4-15-content-addressed-new-skill-generation.zh.md) 不依赖 Git source 发布 inactive bundle；[V4.16](evidence/v4-16-capability-absent-retention-canary.zh.md) 对 exact absent parent/whole-Skill 做无 Git paired replay；64 轮稳定 Tool 与原生其余请求 parity | Delivery Outcome exact Skill invocation、existing-Skill improvement Opportunity/Candidate、rework/currency-cost/reuse/retention/negative-transfer/rollback 完整归因；治理包真实 provider assembled 整链、路由质量及长期误晋升/回滚数据缺失 |
...
最新 V4.20 增量通过根级 `pnpm check`（文档、全包 typecheck、测试和构建）；其中
`dsh-gateway` 7 files/23 tests、`dsh-telegram` 7 files/26 tests、`dsh-evolve-web` 2 files/26 tests，
`dsh-evolve` 63 files passed、1 file skipped，303 tests passed、2 skipped。Cache Contract 全通过；Doctor
十一包原生合同 22/22，十一包 clean-profile 最终 tarball 的 add/dump/boot/真实 Session+Goal+Storage+Tool/
dispose/remove/reboot/readback 1/1（46.15 秒）；独立 Doctor packed add/Loader/command/remove 1/1（4.73 秒）。
