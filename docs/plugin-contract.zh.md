...
仅接受尚未封存测试材料、最多 dispatch 一次且结果 uncertain 的传输失败记录；不接受已生成测试材料、无效材料或成功草稿，
防止用重试选择测试题。新记录用确定性的 `retryOf` 绑定直接父记录；原记录不覆盖，每次两个预留均计入预算。
启动校验完整父子关系；并发、重启、换 UTC 日或保留同一授权都不能重复创建子尝试。子尝试再次传输失败时，
...
独立 Host 门禁只接受当前学习与评测策略仍存在的完整语义实验；关闭未来语义评测不使已完成证据失效。
实验必须有四组请求组成可比、裁判校准与判分完整、四个草稿答案均通过、
...
不授予资格。此处只覆盖有限、模型提出的自包含文本任务，不证明文件、工具、外部效果或广泛安全/泛化，也不是模型自评成功。
