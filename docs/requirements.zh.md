...
DSH Web 已投影 `Capability Gap → Skill Opportunity → Candidate → authoring state`，并将 existing-Skill improvement investigation、exact baseline qualification、correction evidence readiness 与 paired structural admission 作为独立队列展示。页面显示证据 Goal 数、Gap/纠正数、baseline provider/source/id/文件数、authoring/admission/holdout/Retention 分区、结构准入 status/reason、baseline→Candidate tree、声明 diff 计数、protected admission 摘要、资格或阻断原因、
