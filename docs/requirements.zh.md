...
Retention、反事实 Canary、持续监测和低风险自动晋升必须直接绑定内部 Candidate/Envelope/Outcome，使用 baseline/candidate 对照、未见样本、负迁移、安全、权限、成本、时延、cache、故障恢复和精确回滚证据。当前 Retention、Canary 与独立 Rollback Gate 的源码/自动化纵切已按该边界重建，但真实 provider、最终 tarball 浏览器故障恢复、长期率和 full paired 门禁仍属 pending；任何阶段都不能复用已删除的静态 target、Feedback Draft、Evaluator Draft 或旧 journal。
