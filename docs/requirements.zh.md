...
> V4.40 已要求第五个独立 Goal 在 Candidate proposer 前由单独治理调用形成 calibrated assembled Retention Case Pack，并与 Holdout 一起内容寻址进入 Candidate 已绑定的 v3 Evaluation Envelope；V4.41 已从权威 improved Holdout 执行该 exact Retention，并以原生 DSH Jobs 恢复、四象限和 paid-uncertain 门禁持久化结果；V4.42 已完成其最终 tarball 浏览器读路径。真实 provider、Canary 与发布资格仍未证明。
> 更新日期：2026-08-21
