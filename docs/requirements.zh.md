...
> 状态：已确认；目标是只安装并运行于 DSH 的原生 out-of-tree 插件套件。现有实现只作为可重审资产，不能限制重构；`dsh-gateway` 已替换旧 Router。ClawHub、市场、运行时 research Candidate、Git Skill source/ref、预选 Skill、静态 Case Pack/Feedback/Evaluator target、Feedback/Evaluator Draft、Shadow 内 proposer、自动 review expiry、旧 Retention/canary 和对应 Commands/Control/Web/attention 表面均已从活动源码删除。自然 Goal→Capability Gap→跨 Goal 内部经验→Skill Opportunity→隔离 whole-Skill Candidate 已形成部分纵切；Shadow 只消费 exact DSH-assembled Candidate 且零 proposer。现有 Skill 的同版本跨 Goal 纠正会绑定调用时封存的完整 Bundle 和受保护纠正证据，自主生成只修改指令文本、完整继承其余目录树的隔离 Candidate，经独立 Host 结构准入后进入 Candidate-blind exact paired holdout；只有 calibration、assembled、composition 与输入完整性同时通过才允许形成 `improved/ambiguous/not-improved/regressed`。V4.41 又只从同一 Candidate 的权威 `improved` Holdout 和生成前独立 Retention Case Pack 执行 exact paired Retention，四 Goal零花费 abstain，五 Goal形成 `retained/ambiguous/not-retained/regressed`；两种评测均在中断后不盲重试，结果无发布权。V4.42 已从最终 tarball 验证 Retention 的 DSH Web 权威展示、reload、断连保留、恢复和卸载。V4.43 由独立 Host gate 在人工批准后发布 inactive existing-Skill Generation，并分步选择未来 Session；同名替换、旧 Session固定、显式回滚、完整二进制 Bundle 和决定冷恢复已有真实 DSH 自动化，评测仍不自授发布权。V4.44 又把同一 Host owner 的 bounded 状态与 approve/reject/promote 接入 Control、固定 Typert Remote 和 DSH Web，Web 无 Store/pointer 旁路；V4.45 已从最终 tarball 和 clean profile 真实验证 approve、inactive 冷恢复、future-only promote、reload、断连恢复与官方卸载。缺失 Skill 使用不安装目标 Skill 的 assembled baseline，其 Retention/Promotion/Canary/Rollback 纵切也无评测自授发布权。两套独立真实 provider、exact 飞书消息、existing-Skill failed-Outcome Canary、长期 outcome 与 Hermes paired 完成前不构成 v0.1 交付。
...
> V4.40 已要求第五个独立 Goal 在 Candidate proposer 前由单独治理调用形成 calibrated assembled Retention Case Pack，并与 Holdout 一起内容寻址进入 Candidate 已绑定的 v3 Evaluation Envelope；V4.41 已从权威 improved Holdout 执行该 exact Retention，并以原生 DSH Jobs 恢复、四象限和 paid-uncertain 门禁持久化结果；V4.42 已完成其最终 tarball 浏览器读路径；V4.43 已增加独立人工发布门、inactive Generation、future-Session 同名替换与显式回滚的 Host/Command/真实 DSH Session 证据；V4.44 已完成同一门的 Control/Remote/Web 自动化；V4.45 已完成最终 tarball 的发布级浏览器生命周期。failed-Outcome Canary 与两套真实 provider 仍未证明。
> 更新日期：2026-08-21
...
Retention、反事实 Canary、持续监测和低风险自动晋升必须直接绑定内部 Candidate/Envelope/Outcome，使用 baseline/candidate 对照、未见样本、负迁移、安全、权限、成本、时延、cache、故障恢复和精确回滚证据。当前缺失 Skill 的 Retention、Canary 与独立 Rollback Gate 源码/自动化纵切，以及 existing-Skill Retention/发布门最终 tarball 浏览器生命周期已按该边界重建；existing-Skill failed-Outcome Canary、缺失 Skill Canary/rollback 最终包浏览器故障恢复、真实 provider、长期率和 full paired 门禁仍属 pending。任何阶段都不能复用已删除的静态 target、Feedback Draft、Evaluator Draft 或旧 journal。
