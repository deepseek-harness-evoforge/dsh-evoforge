...
审批卡会展示当前操作的完整原生工具参数，请核对后再选择允许一次或拒绝。参数无法可靠关联或内容过长时，
飞书只显示通知、不提供批准按钮；请回到 DSH Web 原会话核对并处理，不要把通知当作已批准或已完成。
点击有效审批按钮后，原卡会尝试更新为“已提交允许一次”或“已提交拒绝”，并移除按钮；批准不代表任务已完成。
若平台更新失败或 Host 重启后仍看到旧按钮，不要反复点击；回到 Web 原会话核对实际状态。
