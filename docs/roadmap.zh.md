...
> V4.40 已先修正 Retention 的生成前身份链：第五 Goal 由单独治理调用形成并校准独立 `skill-tree` Case Pack，与 Holdout 一起内容寻址进 Candidate 已绑定的 v3 Evaluation Envelope；下一门是执行 exact existing-Skill Retention，而不是在 Candidate 后选择测试。
> 更新日期：2026-08-21
...
Existing-Skill improvement 与 missing-Skill Opportunity 分轨：同一 invocation-content hash 在至少两个不同 Goal 中的去重明确纠正可形成独立调查；不同 hash、同 Goal、legacy 或重复 Signal abstain。该 hash 不代表完整 Bundle，调查固定无因果/无发布权，不进入 capability-absent Envelope。V4.33 已在原生调用边界封存完整内容寻址目录 Bundle；V4.34 重新核对调查并把每个 exact correction 的 `(Session, invocation seq)` 逐一解析到同一 revalidated baseline id，missing 等待，漂移/损坏/错配/多 Bundle invalid；V4.35 通过官方 Feedback/Session 服务把至少四 Goal 当前纠正预分为 authoring/admission/holdout/可选 Retention；V4.38 已在 proposer 前由不同 model identity 的治理作者只用 exact baseline 与唯一 protected holdout 形成完整 `skill-tree` known-bad/known-correction/evaluator Case Pack，零 proposer calibration 后原子安装 Envelope，budget/uncertain/calibration failure 均先于 Candidate fail closed；V4.36 随后由受保护作者自主提出 bounded instruction diff，Host 完整继承 baseline 的 references/assets/binary 并落内容寻址、无发布权 Candidate；V4.37 再由独立 Host 门禁重验 exact baseline/Candidate 双树、声明 diff 和 protected admission identity，原生 Jobs 可恢复，Web 分栏显示 `qualified-for-holdout`。V4.39 由另一原生 Jobs 调度器消费 exact Admission、immutable baseline、Candidate 和 Candidate-blind Envelope，在相同 assembled DSH Trial 中执行完整双树；只有 calibration/assembled/composition/integrity 全部通过才按 `fail/pass` 四象限持久判决，输入漂移和未知付费结果 fail closed。最终 tarball 真实浏览器已验证读取、reload、断连保留快照、同端口恢复与卸载。下一步推进 existing-Skill Retention、Canary、晋升和回滚，并以两套独立真实 provider 跑出效果证据。

V4.40 将同一 Evidence Seal 的可选第五 Goal 交给第二次 Candidate 不可见治理调用，单独生成和校准 Retention Case Pack；v3 Evaluation Envelope 同时绑定 Holdout/Retention 输入摘要与内容 hash，Candidate 仍只绑定一个不可替换的 pre-Candidate Envelope id。四 Goal及 legacy v2 明确无 Retention，任一付费 role 未知都变为 durable `uncertain`。下一纵切以 `ExistingSkillRetentionEvaluation.evaluate()/scan()` 消费 V4.39 `improved` verdict 与该 exact Retention partition，接入原生 Jobs 后再推进 Canary、future-Session 晋升和回滚。
...
existing-Skill Retention/Canary/晋升/回滚、correction/outcome 的因果证明，以及 rework/currency-cost/reuse/retention/negative-transfer/rollback 的完整归因、内部 Candidate 治理包的两套独立真实 provider outcome、模型缺口质量、迁移/遗忘/
