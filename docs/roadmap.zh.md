...
LC-1 已完成：`dsh-goal-continuity` 让部署者以 exact Session allowlist 预授权冷恢复后的原生 active
...
P1.9 已实现 Evaluator Draft 最窄闭环：一次显式 Author 只产生私有、不可执行的
内容寻址 Draft；host 固定生成 manifest/known-bad，人工另行批准 exact hash 后才允许 sealed
qualification。Qualified Case Pack 仍不能直接触发 Shadow、Candidate 或 Promotion，Commands/Web
只传 signal id + 静态 target id，普通 Session 模型表面保持逐字段相等。真实 `SIGKILL`、真实
DSH assembled qualification、packed lifecycle 与浏览器确认/取消流程均已通过；证据见
[P1.9](evidence/p1-9-private-evaluator-draft.zh.md)，契约与决策见
[P1.9](architecture/p1-9-evaluator-draft-contract.zh.md)与
[ADR-0029](adr/0029-generated-evaluators-remain-inactive-until-human-qualified.md)。
...
Shadow。新增动作复用 P1.8 Shadow launcher/journal/Jobs/review，只从 P1.9 host journal 恢复 exact
input，并保留独立付费确认；契约与实现证据见
[P1.10](architecture/p1-10-qualified-shadow-handoff.zh.md)与
[ADR-0030](adr/0030-qualified-case-packs-enter-shadow-only-through-a-new-explicit-action.md)、
[P1.10 证据](evidence/p1-10-qualified-shadow-handoff.zh.md)。

P1.11 先验证最直接的抗遗忘假设，不建设历史 Case 平台：对一个 completed/reviewable Shadow 的
exact Candidate，用一个独立 prior Case Pack 重放 sealed calibration 与 baseline/Candidate Trial。
结果为 `retained|regressed|incomplete`，proposer 调用固定为 0，且不改变 active Skill 或 release。
回归/保留、篡改、`SIGKILL` 与 packed CLI 均有前向测试；契约与证据见
[P1.11](architecture/p1-11-exact-retention-gate.zh.md)、
[ADR-0031](adr/0031-retention-is-an-exact-zero-proposer-gate-before-a-case-platform.md)与
[P1.11 证据](evidence/p1-11-exact-retention-gate.zh.md)。

P1.12 可选把 exact Retention 接入既有 clear-instruction 自动 policy，而不自动建设 Case 选择/执行
平台。静态 host roots 中 missing、incomplete、regressed、篡改、symlink 或冲突证据均留在人工
review；真实 P1.11 retained report 出现后，同一 supervisor 重评并只激活 future Session。契约与
证据见 [P1.12](architecture/p1-12-opt-in-retention-auto-promotion-gate.zh.md)、
[ADR-0032](adr/0032-retention-evidence-is-an-opt-in-auto-promotion-gate.md)与
[P1.12 证据](evidence/p1-12-opt-in-retention-auto-promotion-gate.zh.md)。

P1.13 消除上述门前的机械 CLI 断点，但保持最小策略：每个 allowlisted Skill 最多一个静态、exact
prior Case Pack Target；配置本身授权 evaluator 成本。既有 supervisor 每轮最多执行一个，复用 native
Jobs/P1.11/P1.12；retained 才晋升，regressed/incomplete/uncertain 留在 review，potentially effectful
执行崩溃后不自动重试。契约与证据见
...
P1.15 给 P1.14 增加最小长期成本熔断：每个自动 Target 默认 1 次、最多 20 次/UTC 日。一次自动
Signal 在私有 Draft、Job 和 proposer 前先写入当日 durable reservation；崩溃不退款，同一 Signal
幂等复用。额度耗尽或 journal 不可信只停止自动入口，逐次人工 Shadow 和原 Session 不受影响。
每个 Target 只保留一个最多 20 项的 `current.json`；Commands/Web 显示 bounded used/limit/remaining，
不增加 Session composition。契约、决策与证据见
[P1.15](architecture/p1-15-automatic-evolution-budget.zh.md)、
[ADR-0035](adr/0035-automatic-evolution-reserves-budget-before-paid-launch.md)与
[P1.15 证据](evidence/p1-15-automatic-evolution-budget.zh.md)。

P1.16 消除全新失败在 evaluator author 前的逐条命令：默认关闭的 `automaticEvaluatorTargets` 只在
明确纠正恰好匹配一个静态 Skill 时，先消费 P1.15 日预算，再复用 P1.9 生成私有、不可执行 Draft。
人工 qualification、Qualified Shadow、Promotion 与 rollback 权限不变；同一 Skill 不能同时启用
P1.14，避免重复外部调用。`SIGKILL` 后不重复付费请求，原 Session 与 64 轮请求前缀不变。契约、
决策与证据见 [P1.16](architecture/p1-16-automatic-evaluator-draft.zh.md)、
[ADR-0036](adr/0036-explicit-correction-may-create-one-inactive-evaluator-draft.md)与
[P1.16 证据](evidence/p1-16-automatic-evaluator-draft.zh.md)。

P1.17 减少人工已完成语义审查后的机械点击，但不自动 qualification：用户可选择
`qualify-shadow <note>`，一次确认 exact generated-code execution 和“仅成功后”的付费 Shadow。
qualification 失败时 proposer 为 `0`；成功后复用 P1.10 journal；原分步动作和独立 Promotion 均保留。
契约、决策与证据见 [P1.17](architecture/p1-17-human-approved-qualify-and-shadow.zh.md)、
[ADR-0037](adr/0037-one-human-action-may-qualify-and-start-shadow.md)与
[P1.17 证据](evidence/p1-17-human-approved-qualify-and-shadow.zh.md)。

P1.18 控制 burst correction 的自动成本与 review 噪声：P1.14/P1.16 在预算前读取已有 Evaluator
Draft、Shadow journal 与 actionable Review Candidate；同一 Skill 尚未 disposition 时不预留额度、
不发第二次 provider 请求，Signal 留在原 store 后续再查。它不新建队列/lease，不限制人工动作，
状态不可读 fail closed。契约、决策与证据见
[P1.18](architecture/p1-18-per-skill-automatic-inflight-gate.zh.md)、
[ADR-0039](adr/0039-automatic-evolution-keeps-one-unresolved-path-per-skill.md)与
[P1.18 证据](evidence/p1-18-per-skill-automatic-inflight-gate.zh.md)。

P1.19 解决 P1.18 的长期停滞边界：只有 Automatic Feedback Shadow 启动且结论为 `review` 的
Candidate 才有默认 168 小时的窗口；下一条 Signal 到来时先写 existing durable rejection，再重新执行
P1.18。人工、`promote`、未激活和不可读状态均不自动过期；没有 timer、通知、新队列或模型调用。见
[P1.19](architecture/p1-19-automatic-ambiguous-review-expiry.zh.md)、
[ADR-0040](adr/0040-automatic-ambiguous-review-has-a-bounded-window.md)与
[P1.19 证据](evidence/p1-19-automatic-ambiguous-review-expiry.zh.md)。

P1.20 只补 P1.19 的用户可解释性：Review Inbox 从同一事实派生 exact `eligibleAt`、当前 eligible 状态
和 `next-same-skill-automatic-signal` 触发语义，Commands/Web 一致显示。Web 显式刷新同时重读当前
detail，已处置对象清除陈旧表单并安全失败；没有 timer、polling、新状态或模型调用。见
...
P2C.3 在真实交付中消除 CI pending 后的机械 Agent 重试：部署者可再配置有界
`draftPrCheckWait`，只在同一次 active `complete_delivery` 内重复读取 exact-head rollup。push/PR
查找不重复；失败、wrong-head、取消或不可信查询立即停止；timeout 保持 Goal active，重试仍以
GitHub 事实幂等恢复。它不增加 Tool/Skill/Prompt、后台 watcher、CI journal 或模型调用。证据见
[P2C.3](evidence/p2c-3-bounded-draft-check-wait.zh.md)与
...
forge、required-only 规则、CI 日志诊断与后台等待只在真实需求数据证明后扩展。全局拦截所有
...
P3.1 再增加一个单用途组合包：`dsh-evolve-telegram` 在既有 Evolve supervisor settle 或启动补扫时，
把待处理 Candidate/Evaluator Draft 发送到该 exact 私聊。它不建 timer、通用通知 SPI 或 inline 审批；
确定性 notice id 复用 `dsh-telegram` journal，原 Session 不等待，普通 Session token 增量仍为 0。
