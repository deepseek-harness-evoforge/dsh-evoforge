...
Existing-Skill improvement 与 missing-Skill Opportunity 分轨：同一 invocation-content hash 在至少两个不同 Goal 中的去重明确纠正可形成独立调查；不同 hash、同 Goal、legacy 或重复 Signal abstain。该 hash 不代表完整 Bundle，调查固定无因果/无发布权，不进入 capability-absent Envelope。V4.33–V4.38 依次完成调用边界完整 baseline Bundle、同一 baseline 资格、protected evidence seal、whole-tree Candidate、exact 双树结构准入和 Candidate-blind Holdout 治理。V4.39 由独立原生 Jobs 调度器执行 exact assembled paired Holdout；只有 calibration/assembled/composition/integrity 全部通过才按 `fail/pass` 四象限持久判决，输入漂移和未知付费结果 fail closed，最终 tarball 浏览器已验证读取、reload、断连恢复与卸载。V4.41 又只消费同一 Candidate 的权威 `improved` Holdout 和生成前独立 Retention partition，重验 baseline/Candidate、两套 Case Pack 与完整 Trial 门禁后持久给出 retained/ambiguous/not-retained/regressed；四 Goal不花费，付费结果未知不重试。V4.42 已完成 Retention Web/真实浏览器；V4.43 已增加独立 existing-Skill Host release gate、持久人工决定、inactive Generation、同名 future-only Session 选择与显式回滚。下一步接入 Control/Remote/Web 和最终 tarball 浏览器，再推进 failed-Outcome Canary，并以两套独立真实 provider 跑出效果证据。
