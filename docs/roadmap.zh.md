...
> V4.40 已先修正 Retention 的生成前身份链；V4.41 已由 `ExistingSkillRetentionEvaluation.evaluate()/scan()` 消费 authoritative improved Holdout 与该 exact Retention partition，并接入原生 DSH Jobs。下一门是权威 DSH Web/真实浏览器，再推进 Canary、future-Session 晋升和回滚。
> 更新日期：2026-08-21
...
Existing-Skill improvement 与 missing-Skill Opportunity 分轨：同一 invocation-content hash 在至少两个不同 Goal 中的去重明确纠正可形成独立调查；不同 hash、同 Goal、legacy 或重复 Signal abstain。该 hash 不代表完整 Bundle，调查固定无因果/无发布权，不进入 capability-absent Envelope。V4.33–V4.38 依次完成调用边界完整 baseline Bundle、同一 baseline 资格、protected evidence seal、whole-tree Candidate、exact 双树结构准入和 Candidate-blind Holdout 治理。V4.39 由独立原生 Jobs 调度器执行 exact assembled paired Holdout；只有 calibration/assembled/composition/integrity 全部通过才按 `fail/pass` 四象限持久判决，输入漂移和未知付费结果 fail closed，最终 tarball 浏览器已验证读取、reload、断连恢复与卸载。V4.41 又只消费同一 Candidate 的权威 `improved` Holdout 和生成前独立 Retention partition，重验 baseline/Candidate、两套 Case Pack 与完整 Trial 门禁后持久给出 retained/ambiguous/not-retained/regressed；四 Goal不花费，付费结果未知不重试。下一步补 Retention Web/真实浏览器，再推进 existing-Skill Canary、晋升和回滚，并以两套独立真实 provider 跑出效果证据。

V4.40 将同一 Evidence Seal 的可选第五 Goal 交给第二次 Candidate 不可见治理调用，单独生成和校准 Retention Case Pack；v3 Evaluation Envelope 同时绑定 Holdout/Retention 输入摘要与内容 hash，Candidate 仍只绑定一个不可替换的 pre-Candidate Envelope id。V4.41 的 `ExistingSkillRetentionEvaluation.evaluate()/scan()` 已消费 V4.39 权威 `improved` verdict 与该 exact Retention partition，并由原生 DSH Jobs 负责实时/冷启动唤醒；它没有发布权。下一纵切先完成 Web/真实浏览器，再推进 Canary、future-Session 晋升和回滚。
