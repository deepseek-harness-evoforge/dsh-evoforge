...
Self-discovery is internal experience learning. `ExperienceDrivenSkillOpportunityDiscovery` reads only durable, Goal-linked DSH Capability Gaps. One same-name pattern becomes an eligible `SkillOpportunity` only inside one Workspace and after at least two distinct Goal ids. Same-Goal retries, missing Goal identity, cross-Workspace evidence, and insufficient observations abstain. External markets, ClawHub, GitHub, documentation sites, and Agent Skills indexes are not queried by this path.

`selfDiscoveryPolicies` optionally authorizes `{ id, workspaceId, runRoot, maxAttemptsPerUtcDay }`. It deliberately has no `skill`, source, path, Agent, workflow, or route selector. The Skill identity is derived from the internal Opportunity. A native DSH Job receives only bounded Goal/Gap evidence and asks the configured author model for one instruction-only Agent Skill bundle containing root `SKILL.md` and optional one-level `references/*.md`. The Host validates canonical paths, identity, size, regular-file-only content, and no scripts; it hashes model/input/artifact/tree provenance, stores the bundle privately, and emits an inactive/quarantined/unevaluated/never-executed Candidate. One reconciliation schedules at most one Job. A crash after a possibly paid request becomes `uncertain` and is never blindly retried.

The main runtime has no external discovery source configuration, runtime Web research, research Holdout/revision policy, installer, activator, or release interface. DSH Web projects the exact `Capability Gap → Skill Opportunity → Candidate` flow plus phase, evidence counts, cost, digests, and governance state. Candidate bodies and private paths never cross the adapter. Ecosystem research is a design-time or frozen-benchmark activity; any future external package acquisition must be a separately named, explicitly authorized trusted-import feature.
