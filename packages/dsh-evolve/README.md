...
`dsh-evolve` is an evidence-driven evolution extension for DeepSeek Harness. It currently contains twenty-five deliberately separate lanes:
...
- **P1.21 Parent Generation Outcome Comparison** shows bounded active-versus-parent delivery counts as descriptive evidence without turning correlation into a release verdict.
...
P1.21 reuses the existing bounded P2D.1 Delivery Outcome records to count the
active Generation and its exact parent or native DSH in the same host scan.
Commands and Web show the three-state counts with an explicit non-causal
disclaimer. It adds no outcome fields, persistence, model call, Session surface,
percentage claim, promotion, or rollback action. See the
[P1.21 contract](../../docs/architecture/p1-21-parent-generation-outcome-comparison.zh.md).
