...
class DomainInteractionGenerationEvidenceVault implements InteractionGenerationEvidenceVaultV1 {
  ...
  retain(rawReceipt: InteractionGenerationEvidenceReceiptV1): Promise<void> {
    ...
    try {
      result = this.stageReceipt(id, identity, receipt, generationMaxRecords)
    } catch (error) {
      return Promise.reject(error)
    }
    this.trackAcceptedWrite(id, result)
    return result
  }
  ...
  private async pruneConfirmedWorkspace(
    ...
    const resolved = [...table.entries()].flatMap(([id, durable]) => {
    ...
    for (const { id: pruneId, durable: pruneRecord, projectionToken } of oldest) {
      ...
      const currentDurable = table.get(pruneId)
  ...
  async resolveGenerationEvidence(
    ...
    try {
      ...
      throw new Error('Interaction Generation evidence read failed', { cause: error })
    }
    ...
    if (workspacePrunes !== undefined) {
      ...
      try {
        ...
        throw new Error('Interaction Generation evidence read failed after quota pruning', {
          cause: error,
        })
      }
      if (record === undefined) return abstained('evidence-unavailable')
  ...
  private async persistConflict(
    ...
    try {
      ...
      const unavailable = new Error(
        `Interaction Generation evidence '${record.id}' conflict tombstone was not committed: ${errorMessage(error)}`,
        { cause: error },
      )
      this.unavailableError = unavailable
      throw unavailable
