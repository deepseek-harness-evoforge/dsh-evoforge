...
describe('EvolutionControlPlane', () => {
  it('projects one bounded browser snapshot without private host paths or proposal content', async () => {
    ...
    const control = new EvolutionControlPlane({
      ...
      discovery: {
        listCandidates: () => [{
          ...
          version: { kind: 'git-tree' as const, commit: 'a'.repeat(40), treeHash: 'b'.repeat(40) },
          ...
          permissions: { declared: false, executableContent: true, externalEffects: 'unknown' as const },
          safety: {
          ...
          schemaVersion: 1 as const,
          id: '1'.repeat(64),
          discoveredAt: 1_786_896_100_100,
          gapId: 'd'.repeat(64),
          workspaceId: WORKSPACE_ID,
          requestedSkill: 'release-native-extension',
          description: 'Publish a verified release.',
      ...
      researchRevisions: {
        scan: vi.fn(async () => ({
          ...
          runs: [{
            ...
            researchDigest: 'b'.repeat(64),
            parentTreeHash: 'c'.repeat(64),
            inputDigest: '1'.repeat(64),
            reviserIdentityHash: '2'.repeat(64),
            createdAt: '2026-08-18T00:00:02.000Z',
            updatedAt: '2026-08-18T00:00:03.000Z',
            cost: { modelCalls: 1 as const, inputTokens: 144, outputTokens: 55 },
    ...
    expect(overview).toMatchObject({
      ...
      skillDiscovery: {
        ...
        candidates: [{
          ...
          source: { id: 'local-curated', kind: 'local-git' },
          version: { commit: 'a'.repeat(40), treeHash: 'b'.repeat(40) },
          contentHash: 'c'.repeat(64),
    ...
    expect(JSON.stringify(overview)).not.toContain('revision/provider-model')
