...
- id: evoforge-resident
  ...
  config:
    ...
    # Optional: this is already the default for resident services, so crash recovery does not open another tab.
    noOpen: true
```

`noOpen` defaults to `true`. A resident service can restart after a crash, and opening the DSH Web handoff on every
restart would create duplicate browser tabs. Set `noOpen: false` only when this exact service is intentionally responsible
for opening a browser on each start.
...
- unit 直接执行 exact Node + DSH JavaScript entry，不经过 shell 或 `PATH`；`noOpen` 默认只向目标应用追加
  `--no-open`（显式设为 `false` 才取消），不会改变 profile、Session 或 Gateway 路由；
