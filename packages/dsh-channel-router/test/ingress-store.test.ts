...
describe('channel ingress store', () => {
  it('persists intent, exact duplicate outcome, and rejects identity drift', async () => {
    ...
    await expect(store.begin(intent.id, 110)).resolves.toMatchObject({ status: 'executing' })
    await expect(store.settleCommand(intent.id, {
      kind: 'success',
      text: 'Goal active.',
    ...
    await expect(store.prepare({ ...intent, now: 130 })).resolves.toMatchObject({
      created: false,
      record: { status: 'settled' },
    })
    await expect(store.prepare({ ...intent, contentHash: 'd'.repeat(64), now: 140 }))
  ...
  it('recovers a command interrupted at the effect boundary as uncertain', async () => {
    ...
    const first = await openChannelIngressStore(facility)
    await first.prepare(intent)
    await first.begin(intent.id, 110)
    await first.close()

    const resumed = await openChannelIngressStore(facility)
  ...
  it('fails closed instead of exceeding the journal bound with a live effect', async () => {
    const store = await openChannelIngressStore(memoryFacility(), { maxRecords: 1 })
    await store.prepare(intent)
    await expect(store.prepare({ ...intent, id: 'd'.repeat(64), eventHash: 'e'.repeat(64) }))
      .rejects.toThrow('journal is full')
    await store.begin(intent.id, 110)
    await store.settleCommand(intent.id, { kind: 'success' }, 120)
    await expect(store.prepare({
      ...intent, id: 'd'.repeat(64), eventHash: 'e'.repeat(64), now: 130,
    })).resolves.toMatchObject({ created: true })
    expect(store.get(intent.id)).toBeUndefined()
  })
})
