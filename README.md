...
V4.53 已把同一 exact Skill 版本在至少两个不同 Goal 上的唯一 latest failed 投影成可撤回的失败上下文调查；同 Goal retry、后来恢复、missing/unknown 和 latest 冲突不会凑数。Command 与 DSH Web 明示这只是因果复核请求，没有 Candidate、晋升、回滚或发布权。Host/Control/Remote/Web 与测试夹具已实现，最终 tarball 的真实浏览器生命周期仍待验证，因此尚未记为 verified。
