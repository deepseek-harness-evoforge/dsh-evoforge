...
V4.39 最终 tarball 已在真实 DSH Web 验证读取、reload、Host 断连保留快照、同端口恢复和卸载；V4.41 目前完成 exact Retention 自动化与 Host contract，Retention 的 DSH Web 最终 tarball 真实浏览器仍待下一增量。existing-Skill Canary/晋升/回滚、两套独立真实 provider、长期 Outcome、真实飞书 exact route、Hermes paired benchmark 与长期误晋升/负迁移/误回滚证据仍未完成。
