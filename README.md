...
当前活动进化纵切只使用 DSH 内部经验：Goal-linked Gap 与跨 Goal Skill Opportunity 经生成前证据密封后，产生隔离、内容寻址的 whole-Skill Candidate；Candidate 不可见的治理面分别生成 admission、holdout 和可选 Retention Case Pack。Shadow 只消费 exact DSH-assembled Candidate 且零 proposer；Envelope v5 的第五个独立 Goal 会在同一 DSH Job 内执行内容寻址 Retention，持久得出 `retained`/`regressed`/`incomplete`。独立 Host Promotion Eligibility 在每次晋升前重验 Review、Generation、Lineage、Shadow 和唯一 exact Retention，只允许 `retained` 的 Generation 对未来 Session 生效。失败的 durable Outcome 可唤醒另一原生 DSH Job，以 exact Retention Case Pack 对活动 Candidate 做内容寻址反事实 Canary，并只产出 `keep`/`review`/`rollback-eligible`；Candidate、Retention 与 Canary 均无 pointer 写权限。独立 Future-Session Rollback Gate 现可在人工确认后重验 exact Canary，并以 Store 临界区内 expected-active compare 精确回到 parent/native；Command、Remote、Web 共用该门，既有 Session 不漂移。明确纠正只从 feedback 目标回答的唯一 durable Skill invocation、当时可见内容哈希与 exact Goal id/revision 关联；existing-Skill 路径会封存调用时完整 Bundle，要求全部 correction reference 重验为同一 baseline，并在至少四个不同 Goal 后通过官方 Feedback/Session 服务把当前纠正文与 durable Goal/请求预分为 authoring/admission/holdout/可选 Retention。proposer 只见 authoring，Web 只见 evidence id、计数与阻断原因；该证据仍无 Candidate、安装或发布权。用户不选路径、Agent、workflow、Skill 或来源；产品不建设运行时外部 Skill 搜索、获取、下载、导入或市场。该纵切仍只标记为 `implemented`：Canary/rollback 的真实 provider 与最终 tarball 浏览器故障恢复、长期 Outcome、existing-Skill protected whole-tree author/Candidate/paired evaluation、真实飞书 exact route、Hermes paired benchmark 与长期误晋升/负迁移/误回滚证据仍未完成。
