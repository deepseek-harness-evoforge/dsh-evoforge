...
`dsh-evolve-telegram` bridges actionable Candidate and Evaluator Draft states to that existing exact
private chat. It scans once on load and after the existing supervisor settles, with no timer or
second state machine. Messages contain only bounded host facts, an exact id, and a copyable
`/evolve` inspection command; they are never approvals and the originating Session keeps running.
Deterministic ids reuse Telegram's durable journal across reload and restart. The bridge adds no
Tool, Skill, prompt, Command, model call, or normal-Session token.

`dsh-goal-continuity` lets a deployment pre-authorize exact persisted Session ids to rearm a still-active
native Goal after cold resume. It relies on native Goal revisioning, round limits, the Goal driver and
