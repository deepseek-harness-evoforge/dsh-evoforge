...
> **Pre-alpha: do not use for automatic promotion.** Only the P0A.1 safety tracer of `dsh-evolve shadow` is implemented and tested. Trial execution, improvement evaluation, generations, promotion, rollback, continuity, and UI are not complete.
...
The current tracer provides deterministic Skill and case-pack hashes, owned-path and symlink boundaries, concurrent-mutation detection, token-budget enforcement, minimal evidence, and explicit incomplete results. It adds no provider, tool, prompt, or catalog entry to normal DSH sessions, so its current normal-session token and KV-cache delta is zero.
