...
- **V5.210**: Made the resident Gateway and outbound coordinator fail closed after `stop()` begins. A later `start()` now
  ...
  (Evolution `313/313`, Gateway `49/49`, Feishu `56/56`, Telegram `38/38`, and all other local suites/contracts). Real
  channels, providers, Hermes paired benchmarking, long-run effects, npm ownership, and the release tag remain blocked.
  See [V5.210 evidence](docs/evidence/v5-210-gateway-terminal-start-2026-09-04.zh.md).
...
- **V5.208**: Snapshotted Skill Candidate and Existing-Skill Candidate inputs before asynchronous Storage persistence. Caller
  ...
  `56/56`, Telegram `38/38`, and all other local suites/contracts). Real channels, providers, Hermes paired benchmarking,
  long-run effects, npm ownership, and the release tag remain blocked. See [V5.208 evidence](docs/evidence/v5-208-candidate-input-snapshot-2026-09-04.zh.md).

- **V5.207**: Snapshotted content-addressed Generation publish, Session pin/native fallback, and Existing-Skill Release
  decision inputs before they enter the asynchronous Storage queue. Caller mutations after submission can no longer alter
  persisted hashes, lineage, or decision content; clone failures reject the write. Added a real Storage regression and
  reran the latest-DSH-audited root check with `CHECK_RC=0` (Evolution `311/311`, Gateway `48/48`, Feishu `56/56`,
  Telegram `38/38`, and all other local suites/contracts). Real channels, providers, Hermes paired benchmarking, long-run
  effects, npm ownership, and the release tag remain blocked. See [V5.207 evidence](docs/evidence/v5-207-content-addressed-input-snapshot-2026-09-04.zh.md).
...
- **V5.204**: Serialized concurrent `start()` and `dispose()` calls inside the Feishu, static Telegram, and Telegram
  ...
  the latest DSH alpha.5-supported root check passed with `CHECK_RC=0` (Evolution `309/309`, Gateway `47/47`, Evolve Web
  `27/27`, Control Center `5/5`, Doctor `40/40`, Goal Continuity `12/12`, GitHub Review `27/27`, and remaining local gates).
  Real channels, providers, Hermes paired benchmarking, long-run effects, npm ownership, and the release tag remain blocked.
  See [V5.204 evidence](docs/evidence/v5-204-channel-start-dispose-concurrency-full-check-2026-09-04.zh.md).
...
- **V5.193**: Made `dsh-gateway` startup self-cleaning. Any route or persisted-Session validation failure now invokes the
  ...
  the known upstream root-entry build defect remained classified, and the EvoForge root check passed (`CHECK_RC=0`; Gateway
  `43/43`, Evolution `309/309`, Feishu `55/55`, Telegram `36/36`). Real channel/provider/Hermes paired, long-run, npm
  ownership, and release-tag gates remain blocked. See [V5.193 evidence](docs/evidence/v5-193-gateway-startup-failure-cleanup-2026-09-04.zh.md).
